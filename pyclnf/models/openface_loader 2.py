            ...
            for view_idx in range(self.num_views):
                ...
                for landmark_idx in range(self.num_landmarks):
                    try:
                        ...
                        # Read patch expert
                        debug = (landmark_idx < 3)  # Debug first 3 landmarks
                        n_sigmas = len(self.sigma_components[0]) if self.sigma_components else 0
                        patch = self._read_patch_expert(f, self.window_sizes, n_sigmas, debug=debug)
                        view_patches.append(patch)

                        if not patch['empty'] and landmark_idx < 3:
...
def test_ccnf_loader():
    ...
    try:
        ...
        # Verify by loading one patch
        print("\nVerifying exported files...")
        if ccnf.patches and not ccnf.patches[0].get('empty', False):
            patch_0_meta = np.load(f"{output_dir}/patch_00/metadata.npz")
            patch_0_neuron = np.load(f"{output_dir}/patch_00/neuron_00.npz")

            print(f"  Patch 0 metadata keys: {list(patch_0_meta.keys())}")
            print(f"  Patch 0 neuron 0 keys: {list(patch_0_neuron.keys())}")
            print(f"  Neuron weights shape: {patch_0_neuron['weights'].shape}")
