...
class HierarchicalEyeModel:
    ...
    def __init__(self, model_dir: str):
        ...
        # C++ uses [3, 5, 9] but window 9 degrades results - using [3, 5]
        self.window_sizes = [3, 5]
        self.sigma = 1.0  # C++ uses sigma=1.0 for eye model
        self.reg_factor = 0.5  # C++ uses reg_factor=0.5 for eye model
        self.max_iterations = 5  # Iterations per window size
        self.patch_scale = 1.0  # Eye model scale
    ...
    def _compute_sim_transforms(self, eye_landmarks: np.ndarray, params: np.ndarray,
        ...
        ref_params[0] = self.patch_scale  # scale = 1.0 for eye model
    ...
    def refine_eye_landmarks(self, image: np.ndarray, main_landmarks: np.ndarray,
                            ...
                            main_scale: float = 1.0) -> np.ndarray:
        """
        ...
        # Get patch experts for this eye
        patch_experts = ccnf.get_all_patch_experts(self.patch_scale)
        if not patch_experts:
            # No patch experts, use shape-only refinement
            refined_main = main_landmarks.copy()
            for main_idx, eye_idx in mapping.items():
                refined_main[main_idx] = eye_landmarks[eye_idx]
            return refined_main
        ...
        for window_idx, window_size in enumerate(self.window_sizes):
            ...
            sim_ref_to_img, sim_img_to_ref = self._compute_sim_transforms(
                initial_eye_landmarks, params, pdm, side
            )
            ...
            # For NONRIGID phase, recompute response maps at post-RIGID positions
            # This is necessary because RIGID phase can move landmarks significantly
            # Using offset tracking from initial positions leads to sampling wrong areas
            base_landmarks_nonrigid = eye_landmarks.copy()

            # Recompute transforms and response maps at post-RIGID positions
            sim_ref_to_img_nr, sim_img_to_ref_nr = self._compute_sim_transforms(
                base_landmarks_nonrigid, params, pdm, side
            )
            response_maps = self._compute_eye_response_maps(
                scaled_image, base_landmarks_nonrigid, patch_experts, sim_ref_to_img_nr
            )

            # Run NONRIGID phase (shape parameters)
            for iteration in range(self.max_iterations):
                # Compute mean-shift at current positions (offset from post-RIGID base)
                mean_shift = self._compute_eye_mean_shift_with_offset(
                    eye_landmarks, base_landmarks_nonrigid, response_maps, patch_experts,
                    sim_img_to_ref_nr
                )
                ...
                # Transform mean-shifts from reference space to image space (like C++)
                mean_shift_2D = mean_shift.reshape(-1, 2)  # (28, 2)
                mean_shift_2D = mean_shift_2D @ sim_ref_to_img_nr.T
                mean_shift = mean_shift_2D.flatten()

                # DEBUG: First NONRIGID iteration details
                if iteration == 0 and side == 'left' and window_idx == 0:
                    offset_x = eye_landmarks[8, 0] - base_landmarks_nonrigid[8, 0]
                    offset_y = eye_landmarks[8, 1] - base_landmarks_nonrigid[8, 1]
                    ...
                    with open('/tmp/python_eye_nonrigid_debug.txt', 'w') as f:
                        ...
                        f.write(f"base_landmarks Eye_8: ({base_landmarks_nonrigid[8, 0]:.4f}, {base_landmarks_nonrigid[8, 1]:.4f})\n")
    ...
    def _compute_eye_response_maps(self, image: np.ndarray,
        ...
        for lm_idx, patch_expert in patch_experts.items():
            ...
            if lm_idx == 8 and hasattr(self, '_eye8_trace_file') and x < 500 and ws == 3:
