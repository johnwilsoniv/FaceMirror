...
class StableFaceSplitter:
    ...
    def get_stable_midline(self, landmarks, frame_width):
        """Calculate a stable vertical midline that passes through nose bridge and chin"""
        if landmarks is None:
            return frame_width // 2

        # Get nose bridge (point 27) and chin (point 8) coordinates
        nose_bridge = landmarks[27]  # [x, y]
        chin = landmarks[8]  # [x, y]

        # Calculate points along the line between nose bridge and chin
        y_values = np.arange(0, frame_width)  # Generate all possible y-coordinates

        # Handle case where points are perfectly vertical to avoid division by zero
        if abs(nose_bridge[0] - chin[0]) < 1:
            x_values = np.full_like(y_values, nose_bridge[0])
        else:
            # Calculate slope and intercept of the line
            slope = (nose_bridge[1] - chin[1]) / (nose_bridge[0] - chin[0])
            intercept = nose_bridge[1] - slope * nose_bridge[0]

            # Calculate x values for each y using inverse of line equation
            x_values = (y_values - intercept) / slope

        # Find the average x-coordinate along the line
        valid_indices = (x_values >= 0) & (x_values < frame_width)
        if np.any(valid_indices):
            midline = int(np.mean(x_values[valid_indices]))
        else:
            # Fallback to simple average if line calculation fails
            midline = int((nose_bridge[0] + chin[0]) / 2)

        # Add to history for temporal smoothing
        self.midline_history.append(midline)
