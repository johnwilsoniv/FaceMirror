#!/usr/bin/env python3
"""
Re-run comparison test on rotated source videos.
"""
...
def run_cpp_openface(video_path, output_dir, openface_bin):
    ...
    # Find output CSV - use the source filename stem
...
def main():
    """Main function."""
    # Rotated source videos
    source_dir = Path('/Users/johnwilsoniv/Documents/SplitFace/S1O Processed Files/Combined Data')
    videos = [
        source_dir / 'IMG_2737_source.MOV',
        source_dir / 'IMG_5694_source.MOV',
        source_dir / 'IMG_8401_source.MOV',
        source_dir / 'IMG_9330_source.MOV'
    ]
    ...
    print("="*80)
    print("RE-RUNNING ROTATED VIDEOS")
    print("="*80)
    print(f"Processing {len(videos)} rotated videos")
    ...
    for video_path in videos:
        ...
        # Extract original name (without _source suffix)
        ...
        print(f"\n{'='*80}")
        print(f"PROCESSING: {video_name} (rotated source)")
        print(f"{'='*80}")
        ...
        # Generate side-by-side comparison (overwrite existing)
        try:
            ...
            print(f"  🔄 Overwritten: {comparison_path}")
        except Exception as e:
    ...
    print("="*80)
    print(f"Updated comparisons in: {output_base}/")
