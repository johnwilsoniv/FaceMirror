...
\
    ...
    if zone == 'mid':\
        ...
        processed = self.process_midface_combined_score_left(\
            info, zone, aus, left_values, right_values,\
            left_values_normalized, right_values_normalized,\
            confidence_score=self._calculate_confidence_score('left', zone, left_values, right_values, \{\}),\
            zone_paralysis=zone_paralysis,\
            affected_aus_by_zone_side=affected_aus_by_zone_side\
        )\
\
        if processed:\
            logger.debug(f"Left mid face processed successfully with combined score approach")\
...
\
        ...
        processed = self.process_midface_functional_approach_left(\
            info, zone, aus, left_values, right_values,\
            left_values_normalized, right_values_normalized,\
            confidence_score=self._calculate_confidence_score('left', zone, left_values, right_values, \{\}),\
            zone_paralysis=zone_paralysis,\
...
\
        if processed:\
            logger.debug(f"Left mid face processed successfully with functional approach (fallback)")\
            return  # Skip other detection methods for mid face if processed successfully\
\
...
\
    ...
    if 'AU45_r' not in left_values_normalized or 'AU45_r' not in right_values_normalized:\
        return False\
\
    ...
    right_au45 = right_values_normalized['AU45_r']\
\
    # Calculate ratio (left side / right side)\
    ratio = left_au45 / right_au45 if right_au45 > 0 else 0\
\
    # Calculate functional score\
    functional_score = self._calculate_midface_functional_score(left_au45, ratio)\
\
    # Store confidence score\
    info['paralysis']['confidence']['left'][zone] = confidence_score\
\
    # Initialize functional_score tracking if not already present\
    if 'functional_score' not in info['paralysis']['contributing_aus']['left'][zone]:\
        info['paralysis']['contributing_aus']['left'][zone]['functional_score'] = []\
\
    # Debug logging\
    logger.debug(f"LEFT mid face functional assessment: AU45=\{left_au45:.2f\}, ratio=\{ratio:.2f\}, score=\{functional_score:.2f\}, thresholds: complete=\{self.midface_functional_thresholds['complete']\}, partial=\{self.midface_functional_thresholds['partial']\}")\
\
...
\
        ...
        info['paralysis']['contributing_aus']['left'][zone]['functional_score'].append(\{\
            'au': 'AU45_r',\
            'value': left_au45,\
            'other_value': right_au45,\
            'ratio': ratio,\
        ...
        \})\
\
        logger.debug(f"LEFT mid face: COMPLETE paralysis detected with functional approach")\
...
\
        ...
        \})\
\
        logger.debug(f"LEFT mid face: PARTIAL paralysis detected with functional approach")\
    else:\
        # No paralysis detected\
        logger.debug(f"LEFT mid face: NO paralysis detected with functional approach")\
\
...
def process_nonmidface_zones(self, info, zone, aus, left_values, right_values,\
    ...
    side = 'left'  # This function is for left-side detection\
\
    ...
    if zone == 'lower' and hasattr(self,\
        ...
        processed = self.process_lower_face_combined_score_left(\
            info, zone, aus, left_values, right_values,\
            left_values_normalized, right_values_normalized,\
            confidence_score, zone_paralysis, affected_aus_by_zone_side,\
