"""
Model training for ML-based upper face paralysis detection.
...
from upper_face_feature_extraction import prepare_data

logger = logging.getLogger(__name__)

def train_model(features, targets, model_type='random_forest'):
    """
    Train a machine learning model for upper face paralysis detection with improved
...
def save_model_artifacts(model, scaler, feature_importance):
    ...
    try:
        ...
        # Save model
        joblib.dump(model, 'models/upper_face_paralysis_model.pkl')
        logger.info("Model saved to models/upper_face_paralysis_model.pkl")

        # Save scaler
        joblib.dump(scaler, 'models/upper_face_paralysis_scaler.pkl')
        logger.info("Scaler saved to models/upper_face_paralysis_scaler.pkl")

        # Save feature importance
        feature_importance.to_csv('models/upper_face_feature_importance.csv', index=False)
        logger.info("Feature importance saved to models/upper_face_feature_importance.csv")

        # Also save in root directory for backward compatibility
        joblib.dump(model, 'upper_face_paralysis_model.pkl')
        joblib.dump(scaler, 'upper_face_paralysis_scaler.pkl')
        feature_importance.to_csv('upper_face_feature_importance.csv', index=False)
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('upper_face_model_training.log')
        ]
    )

    logger.info("Starting upper face paralysis ML model training...")
    ...
    joblib.dump(gb_model, 'models/gradient_boosting/upper_face_paralysis_model.pkl')
    joblib.dump(gb_scaler, 'models/gradient_boosting/upper_face_paralysis_scaler.pkl')
    gb_importance.to_csv('models/gradient_boosting/upper_face_feature_importance.csv', index=False)
