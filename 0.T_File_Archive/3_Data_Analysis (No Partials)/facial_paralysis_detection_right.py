...
def detect_right_side_paralysis(self, info, zone, aus, left_values, right_values, 
    ...
    au_percent_diffs = []
    for au in aus:
        if au in right_values and au in left_values:
            ...
            if use_normalized and au in right_values_normalized and au in left_values_normalized:
                right_val = right_values_normalized[au]
                left_val = left_values_normalized[au]
                logger.debug(f"Using normalized values for {au} percent diff: R={right_val}, L={left_val}")

            percent_diff = calculate_percent_difference(right_val, left_val)
    ...
    au12_ratio = None
    if zone == 'lower' and 'AU12_r' in right_values and 'AU12_r' in left_values:
        # Use normalized values if available
        if 'AU12_r' in right_values_normalized and 'AU12_r' in left_values_normalized:
            right_au12 = right_values_normalized['AU12_r']
            left_au12 = left_values_normalized['AU12_r']

            if right_au12 > 0 or left_au12 > 0:
                if right_au12 > 0 and left_au12 > 0:
                    au12_percent_diff = calculate_percent_difference(right_au12, left_au12)
                    au12_ratio = min(right_au12, left_au12) / max(right_au12, left_au12)
                else:
    ...
    au25_ratio = None
    if zone == 'lower' and 'AU25_r' in right_values and 'AU25_r' in left_values:
        # Use normalized values if available
        if 'AU25_r' in right_values_normalized and 'AU25_r' in left_values_normalized:
            right_au25 = right_values_normalized['AU25_r']
            left_au25 = left_values_normalized['AU25_r']

            if right_au25 > 0 or left_au25 > 0:
                if right_au25 > 0 and left_au25 > 0:
                    au25_percent_diff = calculate_percent_difference(right_au25, left_au25)
                    au25_ratio = min(right_au25, left_au25) / max(right_au25, left_au25)
                else:
    ...
    au01_ratio = None
    if zone == 'upper' and 'AU01_r' in right_values and 'AU01_r' in left_values:
        # Use normalized values if available
        if 'AU01_r' in right_values_normalized and 'AU01_r' in left_values_normalized:
            right_au01 = right_values_normalized['AU01_r']
            left_au01 = left_values_normalized['AU01_r']

            if right_au01 > 0 or left_au01 > 0:
                if right_au01 > 0 and left_au01 > 0:
                    au01_percent_diff = calculate_percent_difference(right_au01, left_au01)
                    au01_ratio = min(right_au01, left_au01) / max(right_au01, left_au01)
                else:
    ...
    au45_ratio = None
    if zone == 'mid' and 'AU45_r' in right_values and 'AU45_r' in left_values:
        # Use normalized values if available
        if 'AU45_r' in right_values_normalized and 'AU45_r' in left_values_normalized:
            right_au45 = right_values_normalized['AU45_r']
            left_au45 = left_values_normalized['AU45_r']

            if right_au45 > 0 or left_au45 > 0:
                if right_au45 > 0 and left_au45 > 0:
                    au45_percent_diff = calculate_percent_difference(right_au45, left_au45)
                    au45_ratio = min(right_au45, left_au45) / max(right_au45, left_au45)
                else:
    ...
    if zone == 'mid' and self.use_combined_score_for_midface:
        # Get the appropriate values
        if 'AU45_r' in right_values and 'AU45_r' in left_values:
            # Calculate minimum value and ratio
            right_val = right_values['AU45_r']
            left_val = left_values['AU45_r']
            
            if right_values_normalized and left_values_normalized and 'AU45_r' in right_values_normalized and 'AU45_r' in left_values_normalized:
                right_val = right_values_normalized['AU45_r']
                left_val = left_values_normalized['AU45_r']
                
            min_val = right_val  # Since we're checking right side paralysis
            max_val = left_val
            ratio = min_val / max_val if max_val > 0 else 0
            
            # Calculate combined score
            combined_score = self._calculate_midface_combined_score(ratio, min_val)
            logger.debug(f"Right midface combined score: {combined_score:.3f} (min_val={min_val:.2f}, ratio={ratio:.3f})")
            ...
            if combined_score < self.midface_combined_score_thresholds['complete'] and confidence_score >= confidence_thresholds['complete']:
                ...
                info['paralysis']['contributing_aus']['right'][zone]['combined_score'].append({
                    'au': 'AU45_r',
...
def process_midface_original_approach(self, info, zone, aus, values, other_side_values, 
                                     values_normalized, other_side_values_normalized,
                                     confidence_score, zone_paralysis, affected_aus_by_zone_side,
                                     criteria_met, extreme_au, extreme_percent_diff, 
                                     au45_ratio, asymmetry_thresholds, confidence_thresholds, ratio):
    ...
    if criteria_met['extreme_asymmetry']:
        ...
        info['paralysis']['contributing_aus'][side][zone]['extreme_asymmetry'].append({
            'au': extreme_au,
            'right_value': values[extreme_au],
            'left_value': other_side_values[extreme_au],
    ...
    # Special case for AU45_r normalized ratio (mid face)
        ...
        if 'AU45_r' in values and values['AU45_r'] < MID_FACE_FUNCTIONAL_THRESHOLD:
            ...
            info['paralysis']['contributing_aus'][side][zone]['normalized_ratio'].append({
                'au': 'AU45_r',
                'right_value': values_normalized['AU45_r'],
                'left_value': other_side_values_normalized['AU45_r'],
                'ratio': au45_ratio,
                'threshold': asymmetry_thresholds['complete']['ratio'],
                'type': 'Complete',
                'normalized': True
            ...
            if 'AU45_r' in values and values['AU45_r'] >= MID_FACE_FUNCTIONAL_THRESHOLD:
                logger.debug(
                    f"Skipping mid face detection due to functional closure: right AU45_r = {values['AU45_r']:.3f} >= {MID_FACE_FUNCTIONAL_THRESHOLD}")
    ...
    # Special case for borderline cases
        ...
        if 'AU45_r' in values and values['AU45_r'] < MID_FACE_FUNCTIONAL_THRESHOLD:
            ...
            logger.debug(
                f"Skipping mid face borderline case due to functional closure: right AU45_r = {values.get('AU45_r', 'N/A')}")
                ...
                if au in values and au in other_side_values:
                    ...
                    if percent_diff > 70.0 and values[au] < other_side_values[au]:
                        ...
                        info['paralysis']['contributing_aus'][side][zone]['borderline_case'].append({
                            'au': au,
                            'right_value': values[au],
                            'left_value': other_side_values[au],
    ...
    # Determine whether to detect complete, partial, or no paralysis
        ...
        if 'AU45_r' in values and values['AU45_r'] >= MID_FACE_FUNCTIONAL_THRESHOLD:
            ...
            if ratio > MID_FACE_FUNCTION_RATIO_OVERRIDE:
                logger.debug(
                    f"Overriding mid face detection due to functional closure: {values['AU45_r']:.3f} >= {MID_FACE_FUNCTIONAL_THRESHOLD}")
        ...
        for au in aus:
            if au in values:
                ...
                val = values[au]
                if use_normalized and au in values_normalized:
                    val = values_normalized[au]

                # Check if AU is below threshold - handle minimal movement detection
                process_au_minimal_movement(
                    info, zone, au, val, self.paralysis_thresholds[zone]['complete']['minimal_movement'],
                    affected_aus_by_zone_side, side, use_normalized, 'Complete'
                )

                # Check if AU contributes to ratio-based detection
                if au in other_side_values:
                    # Get the appropriate other side value
                    other_val = other_side_values[au]
                    if use_normalized and au in other_side_values_normalized:
                        other_val = other_side_values_normalized[au]

                    # Handle asymmetry detection
                    process_au_asymmetry(
                        info, zone, au, val, other_val, 
    ...
    # Check for partial paralysis
        ...
        for au in aus:
            if au in values:
                ...
                if (val < self.paralysis_thresholds[zone]['partial']['minimal_movement'] and
                        val >= self.paralysis_thresholds[zone]['complete']['minimal_movement']):
                    affected_aus_by_zone_side[side][zone].add(au)
                    if au not in info['paralysis']['affected_aus'][side]:
                        info['paralysis']['affected_aus'][side].append(au)
...
def process_nonmidface_zones(self, info, zone, aus, values, other_side_values, 
                            values_normalized, other_side_values_normalized,
                            confidence_score, zone_paralysis, affected_aus_by_zone_side,
                            criteria_met, extreme_au, extreme_percent_diff, 
                            au12_ratio, au25_ratio, au01_ratio, asymmetry_thresholds, 
    ...
    # Special case for AU12_r normalized ratio
        ...
        info['paralysis']['contributing_aus'][side][zone]['normalized_ratio'].append({
            'au': 'AU12_r',
            'right_value': values_normalized['AU12_r'],
            'left_value': other_side_values_normalized['AU12_r'],
            'ratio': au12_ratio,
            'threshold': asymmetry_thresholds['complete']['ratio'],
            'type': 'Complete',
            'normalized': True
    ...
    # Special case for AU25_r normalized ratio
        ...
        info['paralysis']['contributing_aus'][side][zone]['normalized_ratio'].append({
            'au': 'AU25_r',
            'right_value': values_normalized['AU25_r'],
            'left_value': other_side_values_normalized['AU25_r'],
            'ratio': au25_ratio,
            'threshold': asymmetry_thresholds['complete']['ratio'],
            'type': 'Complete',
            'normalized': True
    ...
    # Special case for AU01_r normalized ratio (upper face)
        ...
        if 'AU01_r' in values and 'AU01_r' in other_side_values:
            if values['AU01_r'] < other_side_values['AU01_r']:
                ...
                info['paralysis']['contributing_aus'][side][zone]['normalized_ratio'].append({
                    'au': 'AU01_r',
                    'right_value': values_normalized['AU01_r'],
                    'left_value': other_side_values_normalized['AU01_r'],
                    'ratio': au01_ratio,
                    'threshold': asymmetry_thresholds['complete']['ratio'],
                    'type': 'Complete',
                    'normalized': True
                ...
                logger.debug(f"Skipping upper face detection - right AU01_r isn't weaker ({values['AU01_r']:.3f} vs {other_side_values['AU01_r']:.3f})")
    # Special case for borderline cases
    elif criteria_met.get('borderline_case', False) and confidence_score >= confidence_thresholds['complete']:
        # Handle borderline cases with specific checks for upper face
        handle_borderline_cases(
            info, zone, aus, values, other_side_values, 
            zone_paralysis, affected_aus_by_zone_side, side
        )
    # Determine whether to detect complete, partial, or no paralysis
    elif ((criteria_met['minimal_movement'] or criteria_met['ratio']) and
          confidence_score >= confidence_thresholds['complete']):
        ...
        if zone == 'upper':
            ...
            if 'AU01_r' in values and 'AU01_r' in other_side_values and 'AU02_r' in values and 'AU02_r' in other_side_values:
                au01_right_weaker = values['AU01_r'] < other_side_values['AU01_r']
                au02_right_weaker = values['AU02_r'] < other_side_values['AU02_r']
        ...
        for au in aus:
            if au in values:
                ...
                if val < complete_thresholds['minimal_movement']:
                    affected_aus_by_zone_side[side][zone].add(au)
                    if au not in info['paralysis']['affected_aus'][side]:
                        info['paralysis']['affected_aus'][side].append(au)
    ...
    # Check for partial paralysis
        ...
        for au in aus:
            if au in values:
                ...
                if (val < partial_thresholds['minimal_movement'] and
                        val >= complete_thresholds['minimal_movement']):
                    affected_aus_by_zone_side[side][zone].add(au)
                    if au not in info['paralysis']['affected_aus'][side]:
                        info['paralysis']['affected_aus'][side].append(au)
        ...
        for au in aus:
            if au in values and au in other_side_values:
                ...
                if percent_diff > 70.0 and values[au] < other_side_values[au]:
                    ...
                    info['paralysis']['contributing_aus'][side][zone]['borderline_case'].append({
                        'au': au,
                        'right_value': values[au] if side == 'right' else other_side_values[au],
                        'left_value': other_side_values[au] if side == 'right' else values[au],
                        'percent_diff': percent_diff,
                        'ratio': values[au] / other_side_values[au] if other_side_values[au] > 0 else 0,
                        'type': 'Complete'
                    })
    ...
    if other_value > 0 and value > 0:
        ...
        if au_ratio < ratio_threshold and value < other_value:
            ...
            info['paralysis']['contributing_aus'][side][zone]['asymmetry'].append({
                'au': au,
                'right_value': value if side == 'right' else other_value,
                'left_value': other_value if side == 'right' else value,
                'ratio': au_ratio,
