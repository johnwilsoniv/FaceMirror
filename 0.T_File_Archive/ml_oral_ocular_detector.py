"""
ML-based Oral-Ocular synkinesis detector.
Uses trained machine learning model to detect mouth-to-eye synkinesis.
"""
...
class MLOralOcularDetector:
    """
    Machine learning-based Oral-Ocular synkinesis detector.
    Detects synkinesis where mouth actions cause unwanted eye movements.
    """
    ...
    def __init__(self):
        ...
        try:
            ...
            if os.path.exists('models/synkinesis/oral_ocular/model.pkl'):
                self.model = joblib.load('models/synkinesis/oral_ocular/model.pkl')
                self.scaler = joblib.load('models/synkinesis/oral_ocular/scaler.pkl')

                # Load feature names if available
                if os.path.exists('models/synkinesis/oral_ocular/feature_importance.csv'):
                    feature_df = pd.read_csv('models/synkinesis/oral_ocular/feature_importance.csv')
                    self.feature_names = feature_df['feature'].tolist()
            # Fall back to root directory
            elif os.path.exists('oral_ocular_synkinesis_model.pkl'):
                self.model = joblib.load('oral_ocular_synkinesis_model.pkl')
                self.scaler = joblib.load('oral_ocular_synkinesis_scaler.pkl')

                # Load feature names if available
                if os.path.exists('oral_ocular_feature_importance.csv'):
                    feature_df = pd.read_csv('oral_ocular_feature_importance.csv')
                    self.feature_names = feature_df['feature'].tolist()

            logger.info("ML Oral-Ocular synkinesis detector initialized successfully")
        except Exception as e:
            logger.error(f"Error initializing ML Oral-Ocular detector: {str(e)}")
            # No fallback - this is an error state that should be fixed
            logger.warning("ML Oral-Ocular detector will not be available")
            self.model = None
            self.scaler = None

    def detect_oral_ocular_synkinesis(self, action, info, side):
        """
        ML-based detection for Oral-Ocular synkinesis.
        ...
        if self.model is None or self.scaler is None:
            ...
            logger.warning(f"ML model not available for Oral-Ocular synkinesis detection")
            return False, 0.0, {'trigger': [], 'response': []}

        try:
            # Only process relevant actions for Oral-Ocular synkinesis (mouth movements)
            relevant_actions = ['BS', 'SS', 'SO', 'SE']
            ...
            if is_detected:
                logger.debug(f"ML Oral-Ocular synkinesis detected on {side} side during {action} with confidence {confidence_score:.3f}")

            return is_detected, confidence_score, contributing_aus

        except Exception as e:
            logger.error(f"Exception in ML detect_oral_ocular_synkinesis: {str(e)}")
    ...
    def _extract_features(self, action, info, side):
        ...
        # Relevant action data for Oral-Ocular synkinesis
        # Trigger AUs (mouth related)
        trigger_aus = ['AU12_r', 'AU25_r']
        # Coupled AUs (eye related)
        coupled_aus = ['AU45_r', 'AU06_r']
        
        # Get opposite side
        opposite_side = 'right' if side == 'left' else 'left'
        
        # Extract features for trigger AUs (mouth actions)
        ...
        # Extract features for coupled AUs (eye actions)
    ...
    def _determine_contributing_aus(self, action, info, side, is_detected):
        ...
        trigger_threshold = 1.8  # Significant mouth action
        coupled_threshold = 1.2  # Significant unwanted eye response
