...
def train_model(features, targets, model_type='random_forest'):
    """
    Train a machine learning model for lower face paralysis detection with improved
...
def save_model_artifacts(model, scaler, feature_importance, model_type='random_forest'):
    """
    ...
    try:
        ...
        if model_type == 'gradient_boosting':
            # Save gradient boosting model to its own files
            os.makedirs('models/gradient_boosting', exist_ok=True)
            joblib.dump(model, 'models/gradient_boosting/lower_face_paralysis_model.pkl')
            joblib.dump(scaler, 'models/gradient_boosting/lower_face_paralysis_scaler.pkl')
            feature_importance.to_csv('models/gradient_boosting/feature_importance.csv', index=False)
            logger.info("Gradient Boosting model artifacts saved to models/gradient_boosting/")
        else:
            # Save random forest model as the default model
            ...
            logger.info("Random Forest model saved to models/lower_face_paralysis_model.pkl")
...
def main():
    ...
    logger.info("Starting lower face paralysis ML model training...")
    ...
    save_model_artifacts(rf_model, rf_scaler, rf_importance, model_type='random_forest')
    ...
    save_model_artifacts(gb_model, gb_scaler, gb_importance, model_type='gradient_boosting')
