...
def run_zone_training_pipeline(zone_key_pipe, zone_config_all_pipe, input_files_all_pipe, class_names_global_pipe):
    ...
    # 1. Select only the features that were selected during training
    # 2. Scale them with the scaler (which was fit on these selected features only)
    try:
        X_test_selected_pipe = X_test_pipe[selected_feature_names_final_pipe]
        X_test_scaled_selected_pipe = scaler_pipe.transform(X_test_selected_pipe)
        ...
        logger.error(f"[{zone_name_display_pipe}] Key error selecting features for test set: {e_key_test_select}. "
                     f"Available columns: {X_test_pipe.columns.tolist()[:10]}... "
                     f"Needed: {selected_feature_names_final_pipe[:10]}...")
        logger.error(
            f"[{zone_name_display_pipe}] Ensure selected_feature_names_final_pipe are present in X_test_pipe columns.")
