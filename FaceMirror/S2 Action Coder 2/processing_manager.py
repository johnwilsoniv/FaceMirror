...
class ProcessingManager(QObject):
    ...
    def __init__(self, timeline_processor, ffmpeg_path, parent=None, whisper_model=None):
        ...
        self.current_whisper_video_path = None  # Track which video is being transcribed
    ...
    def start_whisper_processing(self, video_path):
        ...
        # If a thread is already running, schedule this video to be processed after it finishes
        if self.whisper_thread and self.whisper_thread.isRunning():
            print(f"ProcessingManager: Another transcription is running, will start new one after it finishes...")
            print(f"ProcessingManager: Cancelling current transcription for: {self.current_whisper_video_path}")
            print(f"ProcessingManager: Pending new transcription for: {video_path}")

            # Store the pending video path
            self.pending_whisper_video_path = video_path

            # Tell the current thread to stop
            self.whisper_thread.stop()

            # The _on_whisper_thread_finished handler will check for pending_whisper_video_path
            # and automatically start the new transcription
            return
        ...
        self.current_whisper_video_path = video_path  # Track which video we're processing
        self.pending_whisper_video_path = None  # Clear any pending path
    ...
    def stop_whisper_processing(self, wait=False):
        ...
        # Clear video path tracking when stopping
        self.current_whisper_video_path = None
        self.pending_whisper_video_path = None  # Also clear any pending request
    ...
    def _on_whisper_thread_finished(self):
        ...
        if self.pending_whisper_video_path:
            pending_path = self.pending_whisper_video_path
            self.pending_whisper_video_path = None  # Clear it before starting
            print(f"ProcessingManager: Starting pending transcription for: {pending_path}")
            # Use QTimer to defer the call slightly to ensure clean state
            # Use a strong reference to self to prevent crashes if object is deleted
            try:
                QTimer.singleShot(100, lambda path=pending_path: self._start_pending_transcription(path))
            except RuntimeError as e:
                print(f"ProcessingManager WARNING: Could not schedule pending transcription: {e}")

    def _start_pending_transcription(self, video_path):
        """Helper method to safely start pending transcription from QTimer."""
        try:
            if video_path and os.path.exists(video_path):
                self.start_whisper_processing(video_path)
            else:
                print(f"ProcessingManager WARNING: Pending video path invalid: {video_path}")
        except Exception as e:
            print(f"ProcessingManager ERROR: Failed to start pending transcription: {e}")
    @pyqtSlot()
