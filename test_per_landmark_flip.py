#!/usr/bin/env python3
"""
Test which landmarks need Y-flip and which don't.
"""
...
def main():
    ...
    print("=" * 70)
    print("PER-LANDMARK FLIP ANALYSIS")
    print("=" * 70)
    print("\nDetermining which landmarks need Y-flip:\n")
    ...
    for eye_idx in [8, 10, 12, 14, 16, 18]:
        ...
        flip_map = np.zeros((ws, ws), dtype=np.float32)
        ...
        for dy in range(-half_ws, half_ws + 1):
            for dx in range(-half_ws, half_ws + 1):
                ...
                flip_map[-dy + half_ws, dx + half_ws] = response

        orig_ms_x, orig_ms_y = compute_mean_shift(orig_map)
        flip_ms_x, flip_ms_y = compute_mean_shift(flip_map)

        orig_match = (orig_ms_y > 0) == (cpp_dy > 0) if abs(cpp_dy) > 0.1 else True
        flip_match = (flip_ms_y > 0) == (cpp_dy > 0) if abs(cpp_dy) > 0.1 else True

        if flip_match and not orig_match:
            result = "NEEDS FLIP"
            needs_flip.append(eye_idx)
        elif orig_match and not flip_match:
            result = "NO FLIP"
            no_flip.append(eye_idx)
        elif orig_match and flip_match:
            result = "BOTH OK"
        else:
            result = "NEITHER"

        print(f"Eye_{eye_idx} (main {main_idx}): {result}")
        print(f"  C++ Δy: {cpp_dy:+.4f}")
        print(f"  Original: {orig_ms_y:+.4f} {'✓' if orig_match else '✗'}")
        print(f"  Flip Y:   {flip_ms_y:+.4f} {'✓' if flip_match else '✗'}")
        print()

    print("=" * 70)
    print(f"Summary: Needs flip: {needs_flip}, No flip: {no_flip}")
    print("=" * 70)

    # Check if there's a pattern
    print("\nPattern analysis:")
    print(f"  Eye_8 = outer corner (lateral canthus)")
    print(f"  Eye_10, 12, 14, 16, 18 = eyelid points")
    print(f"\nPossible fix: Don't flip Eye_8, flip all others")
