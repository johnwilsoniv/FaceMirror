...
def train_model_workflow(zone_key_mw, X_train_df_mw, y_train_arr_mw, X_test_df_mw, y_test_arr_mw,
    ...
    # First, create a temporary scaler for feature selection purposes only
    scaler_temp_for_fs = StandardScaler()
    X_train_scaled_mw_full = scaler_temp_for_fs.fit_transform(X_train_df_mw)

    selected_feature_names_mw = feature_names_mw_initial
    selected_feature_indices_mw = list(range(len(feature_names_mw_initial)))  # Track indices
    ...
    if feature_sel_cfg_mw.get('enabled', False):
        ...
        if len(selected_feature_indices_fs) > 0:
            ...
            selected_feature_indices_mw = list(selected_feature_indices_fs)
            logger.info(
    ...
    # NOW create the FINAL scaler fitted on ONLY the selected features
    logger.info(f"[{zone_name_disp_mw}] Creating scaler for {len(feature_names_mw)} selected features...")
    X_train_selected_features_df = X_train_df_mw[feature_names_mw] if isinstance(X_train_df_mw, pd.DataFrame) else pd.DataFrame(X_train_df_mw, columns=feature_names_mw_initial)[feature_names_mw]
    scaler_mw = StandardScaler()
    X_train_scaled_mw_selected = scaler_mw.fit_transform(X_train_selected_features_df)
    ...
    if tuning_cfg_mw.get('enabled', False) and tuning_cfg_mw.get('method') == 'optuna' and OPTUNA_AVAILABLE:
        ...
        # Multi-core optimization: Calculate parallel trials before creating objective
        # Use n_jobs=4 to run 4 trials simultaneously on 8 cores (leaves room for nested parallelism)
        # Each trial's model training (RF, ET, XGB) will use remaining cores
        optuna_n_jobs = min(4, n_jobs_workflow if n_jobs_workflow > 0 else 4)  # Cap at 4 parallel trials
        ...
        objective_fn_mw = create_optuna_objective(
            ...
            optuna_n_jobs=optuna_n_jobs  # Pass n_jobs to optimize XGBoost threading
        )
        ...
        logger.info(f"[{zone_name_disp_mw}] Running Optuna with {optuna_n_jobs} parallel trials (XGBoost using {os.cpu_count() // optuna_n_jobs} threads each)...")
        study_mw.optimize(objective_fn_mw, n_trials=n_trials_mw, n_jobs=optuna_n_jobs, callbacks=optuna_callbacks_list)
    ...
    try:
        # Select only the features that were used for training, then scale them
        X_train_selected_for_analysis = X_train_df_mw[feature_names_mw] if isinstance(X_train_df_mw, pd.DataFrame) else pd.DataFrame(X_train_df_mw, columns=feature_names_mw_initial)[feature_names_mw]
        X_train_scaled_for_analysis_selected = scaler_mw.transform(X_train_selected_for_analysis)  # Scale selected features only
