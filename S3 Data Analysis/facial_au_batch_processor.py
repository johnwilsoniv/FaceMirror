...
# Batch processor for facial paralysis detection.
...
class FacialAUBatchProcessor:
    ...
    def process_patient(self, patient_info, extract_frames=True, expert_data_df=None, debug_mode=False):
        ...
        try:
            ...
            logger.info(f"({patient_id}) Analyzing actions for paralysis...")
            results = analyzer.analyze_all_actions(run_ml_paralysis=True)
            if not results:
            ...
            if results and isinstance(results, dict):
                 ...
                 if first_info: summary_error['Paralysis Detected'] = first_info.get('paralysis', {}).get('detected', 'Error')
    ...
    def process_all(self, extract_frames=True, debug_mode=False, progress_callback=None):
        ...
        if all_patient_summaries:
            ...
            try:
                 ...
                 detection_flags = [c for c in ['Paralysis Detected'] if c in all_cols]
                 ...
                 ordered_groups = id_group + detection_flags + paralysis_details + action_max + action_aus + action_aus_norm
    ...
    def analyze_asymmetry_across_patients(self):
        ...
        logger.info("Analyzing paralysis statistics from summary data...")
        self.analyze_paralysis_statistics()
        logger.info("Aggregate analysis complete.")
        return None


    def analyze_paralysis_statistics(self):
        """Analyzes and logs paralysis statistics across all patients."""
        ...
        logger.info(f"Analyzing paralysis stats for {num_patients} successfully processed patients.")
