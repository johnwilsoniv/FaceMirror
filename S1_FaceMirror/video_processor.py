    ...
    def release(self):
        ...
        try:
            ...
            if self.process.stdin:
                ...
                safe_print(f"    Closed stdin for {filename}, waiting for FFmpeg...")

            # Wait for FFmpeg to finish encoding
            self.process.wait(timeout=30)

            safe_print(f"    FFmpeg finished for {filename}")
...
class VideoProcessor:
    ...
    def _process_frame_batch(self, frame_data):
        ...
        try:
            ...
            if landmarks is not None:
                ...
                # Only generate debug frame if enabled (saves ~0.3-0.5 fps when disabled)
                if config.GENERATE_DEBUG_VIDEO:
    ...
    def process_video(self, input_path, output_dir):
        ...
        safe_print(f"\nVideo details:")
        safe_print(f"- Resolution: {width}x{height}")
        safe_print(f"- Frames: {total_frames}")
        safe_print(f"- Duration: {duration:.1f} seconds")
        safe_print(f"- FPS: {fps}")
        ...
        write_queue = queue.Queue(maxsize=600)  # Buffer ~600 frames (~72MB) - larger for smoother processing
        ...
        safe_print(f"Using {self.num_threads} threads per batch")
        ...
        with tqdm(total=total_frames, desc="Mirroring", unit="frame",
            ...
            # Create thread pool ONCE and reuse for all batches (eliminates batch transition overhead)
            with ThreadPoolExecutor(max_workers=self.num_threads) as executor:
                while current_batch:
                    ...
                    # Submit remaining frames for parallel processing (reusing existing threads)
                    ...
                    # OPTIMIZED GC: Only run full collection every 30 batches
                    # This reduces GC overhead while maintaining memory efficiency
                    gc_start = time.time()
                    if batch_num % 30 == 0:
                        gc.collect()
                    ...
                    if self.debug_mode and batch_num % 10 == 0:
                        ...
                        safe_print(f"    Read:    {total_read_time:.2f}s ({total_read_time/batch_num:.3f}s/batch)")
                        safe_print(f"    Process: {total_process_time:.2f}s ({total_process_time/batch_num:.3f}s/batch)")
                        safe_print(f"    Write:   {total_write_time:.2f}s ({total_write_time/batch_num:.3f}s/batch)")
                        safe_print(f"    Cleanup: {total_cleanup_time:.2f}s ({total_cleanup_time/batch_num:.3f}s/batch)")
                        total_time = total_read_time + total_process_time + total_write_time + total_cleanup_time
                        safe_print(f"    Total:   {total_time:.2f}s")
                        if total_time > 0:
                            safe_print(f"    Breakdown: Read {total_read_time/total_time*100:.1f}% | "
        ...
        if final_queue_depth > 0:
            safe_print(f"Flushing {final_queue_depth} remaining frames to disk...")
        ...
        safe_print("Waiting for writer thread to finish...")
        writer_thread.join(timeout=10.0)  # Wait for thread to finish

        if writer_thread.is_alive():
            safe_print("  Warning: Writer thread still running after 10s timeout")
        ...
        safe_print("Finalizing video encoding (this may take a few seconds)...")

        finalize_start = time.time()

        def finalize_writer(writer, name):
            """Finalize a single writer in a thread"""
            safe_print(f"  Finalizing {name} video...")
            writer.release()
            safe_print(f"  {name} video finalized")
        ...
        safe_print(f"Video encoding finalized in {finalize_elapsed:.1f}s")

        if flush_elapsed > 1.0:
            safe_print(f"Frame flush completed in {flush_elapsed:.2f}s")
        ...
        safe_print("\nOutput files:")
        safe_print("  Mirror videos (Face Mirror 1.0 Output):")
        for f in [str(anatomical_right_output), str(anatomical_left_output), str(debug_output)]:
            safe_print(f"    - {Path(f).name}")
        if str(source_input_path) != str(input_path):
            safe_print("  Source video (Combined Data):")
            safe_print(f"    - {Path(source_input_path).name}")
        safe_print("")
