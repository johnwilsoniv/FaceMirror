...
def auto_detect_device():
    ...
    if config.FORCE_DEVICE is not None:
        safe_print(f"Using forced device: {config.FORCE_DEVICE}")
        return config.FORCE_DEVICE

    # Priority 1: CUDA (NVIDIA GPU)
    if torch.cuda.is_available():
        device_name = torch.cuda.get_device_name(0)
        safe_print(f"NVIDIA GPU detected: {device_name}")
        safe_print("Using CUDA acceleration with PyTorch models")
        return 'cuda'
    ...
    if processor == 'arm' or machine == 'arm64':
        safe_print("Apple Silicon detected (M1/M2/M3)")
        safe_print("Using CPU with ONNX + CoreML Neural Engine acceleration")
        safe_print("Expected performance: 5-20x faster than PyTorch CPU")
    else:
        safe_print(f"Intel/AMD CPU detected ({processor or machine})")
        safe_print("Using CPU with ONNX optimization")
        safe_print("Expected performance: 2-5x faster than PyTorch CPU")
...
def process_single_video(args):
    ...
    def progress_callback(stage, current, total, message, fps=0.0):
        ...
        if stage in ['reading', 'processing', 'writing']:
            if current > 0 and current % 50 == 0:
                ...
                safe_print(f"  [{stage_name}] {current:>6}/{total:>6} frames ({percentage:>5.1f}%){fps_str}{eta_str}", end='\r')
            ...
            # Print completion message
                ...
                stage_name = stage_names.get(stage, stage.title())
                safe_print(f"  [{stage_name}] {current:>6}/{total:>6} frames (100.0%) - Complete          ")
    ...
    try:
                    ...
                    def openface_progress_callback(current_frame, total_frames, processing_fps):
                        ...
                        if current_frame > 0 and current_frame % 50 == 0:
                            ...
                            if processing_fps > 0:
                                ...
                                eta_str = ""
                                fps_str = ""
                            safe_print(f"  [AU Extraction] {current_frame:>6}/{total_frames:>6} frames ({percentage:>5.1f}%){fps_str}{eta_str}", end='\r')

                        # Print completion message
                        elif current_frame == total_frames and total_frames > 0:
                            safe_print(f"  [AU Extraction] {current_frame:>6}/{total_frames:>6} frames (100.0%) - Complete          ")
        ...
        try:
            ...
            safe_print(f"  Warning: Splitter cleanup failed: {e}")
        ...
        try:
            ...
            safe_print(f"  Warning: PyFaceAU processor cache cleanup failed: {e}")
...
def video_processing_worker(input_paths, output_dir, openface_output_dir, debug_mode, device, progress_window):
    ...
    try:
        ...
        safe_print("\n" + "="*60)
        safe_print("INITIALIZING OPENFACE PROCESSOR (ONE-TIME SETUP)")
        safe_print("="*60)
        safe_print("\nPerformance profiling enabled - reports will be saved to Desktop:")
        import datetime
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        safe_print(f"  - face_mirror_performance_{timestamp}.txt")
        safe_print(f"  - face_mirror_performance_{timestamp}.json")
        safe_print()
        ...
        safe_print("PyFaceAU processor initialized and warmed up")
        safe_print("  This will be reused for all videos (eliminates stage delays)")
        safe_print("="*60 + "\n")
...
def workflow_mirror_openface():
    ...
    if not input_paths:
        safe_print("No videos to process (user cancelled or all files skipped)")
        root.destroy()
        return
    ...
    safe_print(f"Available CPU cores: {cpu_count()}")
    safe_print(f"Using device: {device.upper()}")
    ...
    worker_thread.start()
    safe_print("Background processing thread started...")
...
def main():
    ...
    safe_print("\n" + "="*60)
    safe_print(f"S1 FACE MIRROR v{config_paths.VERSION} - FULL PIPELINE")
    safe_print("Face Mirroring + Action Unit Extraction")
    safe_print("="*60)
