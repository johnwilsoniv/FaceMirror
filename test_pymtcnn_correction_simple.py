#!/usr/bin/env python3
"""
Simple test: Apply OpenFace MTCNN correction to raw pyMTCNN output.

Tests whether OpenFace's correction formula improves convergence when applied
to pyMTCNN detections (bypassing the built-in correction).
"""
...
def get_raw_pymtcnn_bbox(detector, frame):
    """Get RAW pyMTCNN bbox before OpenFace correction is applied.

    We access the detector's internal methods to get the raw output.
    """
    # Run the detector's internal detection (before correction)
    # The detector stores raw results before applying correction
    rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

    # Run MTCNN network
    boxes, probs, landmarks = detector.mtcnn.detect(rgb, landmarks=True)

    if boxes is None or len(boxes) == 0:
        return None

    # Return first detection (x, y, w, h format)
...
def main():
    ...
    for video_idx, (video_path, frame_nums) in enumerate(VIDEO_CONFIGS, 1):
        ...
        for frame_idx, frame_num in enumerate(frame_nums, 1):
            ...
            # Get raw pyMTCNN detection
            print(f"    Getting raw pyMTCNN bbox...")
            try:
                original_bbox = get_raw_pymtcnn_bbox(detector, frame)
            except Exception as e:
                print(f"    ✗ Failed to get raw bbox: {e}")
                continue
            ...
            print(f"    ✓ Raw pyMTCNN bbox: {original_bbox}")
