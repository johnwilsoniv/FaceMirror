# mid_face_detector.py (Refactored with DataFrame Scaling)
...
try:
    ...
    MODEL_FILENAMES = {'base_model': os.path.join(MODEL_DIR, 'mid_face_model.pkl'),
                       'base_scaler': os.path.join(MODEL_DIR, 'mid_face_scaler.pkl'),
                       'feature_list': os.path.join(MODEL_DIR, 'mid_face_features.list')} # Add feature list path
...
try:
    feature_list_path_check = MODEL_FILENAMES.get('feature_list', os.path.join(MODEL_DIR, 'mid_face_features.list'))
    ...
    logging.error("Could not import extract_features_for_detection from mid_face_features.py.")
    extract_features_for_detection = None

# Configure logging
logger = logging.getLogger(__name__) # Assuming already configured by main

class MidFaceParalysisDetector:
    """ Detects mid face paralysis using ML model. """
    def __init__(self):
        logger.info("Initializing MidFaceParalysisDetector")
    ...
    def _load_models(self):
        ...
        try:
            ...
            if not all([model_path, scaler_path, features_path]): logger.error("Missing mid-face config paths."); return
            ...
            if paths_exist:
                ...
                logger.info("Mid face artifacts loaded."); assert isinstance(self.feature_names, list), "Feature names not a list"
            ...
            else: logger.error("Mid face artifacts missing."); self.base_model, self.base_scaler, self.feature_names = None, None, None
    ...
    def detect(self, row_data, side, zone):
        ...
        if not all([self.base_model, self.base_scaler, self.feature_names]):
            logger.warning(f"Mid Face components unavailable for {side} {zone}.")
            return 'Error', 0.0, {'error': 'Model/Scaler/Features missing'}
        if extract_features_for_detection is None:
             logger.error("Mid face feature extraction unavailable."); return 'Error', 0.0, {'error': 'Feature func missing'}

        try:
            features_list = extract_features_for_detection(row_data, side, zone)
            if features_list is None: logger.error(f"M-Feat extract fail {side} {zone}"); return 'Error', 0.0, {'error': 'Feat extract failed'}
            if len(features_list) != len(self.feature_names): logger.error(f"M-Feat mismatch {side} {zone}"); return 'Error', 0.0, {'error': 'Feat mismatch'}
            ...
            logger.debug(f"Mid Face Detect - {side} {zone}: Result={result}, Conf={confidence:.3f}")
            return result, confidence, details

        except Exception as e:
            logger.error(f"Error in mid face detect {side} {zone}: {e}", exc_info=True)
