...
class FacialParalysisDetector:
        ...
        if 'AU12_r' in current_side_values and 'AU12_r' in other_side_values:
            ...
            if not has_au25 and current_au12 > 0 and other_au12 > 0:
                ...
                if ((simplified_score < special_threshold and current_au12 < other_au12) or
                    ...
                    # Initialize combined_score structure if not already present
                    if 'combined_score' not in info['paralysis']['contributing_aus'][side][zone]:
                        info['paralysis']['contributing_aus'][side][zone]['combined_score'] = []
            ...
            if 'AU25_r' in current_side_values and 'AU25_r' in other_side_values:
                ...
                if combined_score < partial_threshold and confidence_score >= self.lower_face_confidence_thresholds['partial']:
                    ...
                    if combined_score < complete_threshold and confidence_score >= self.lower_face_confidence_thresholds['complete']:
                        # Complete paralysis
                        ...
                        info['paralysis']['contributing_aus'][side][zone]['combined_score'][-1]['threshold'] = complete_threshold
                    else:
    ...
    def process_lower_face_combined_score_left(self, info, zone, aus, left_values, right_values,
                                               ...
                                               confidence_score, zone_paralysis, affected_aus_by_zone_side,
                                               criteria_met=None, asymmetry_thresholds=None,
    ...
    def detect_paralysis(self, results):
        ...
        for action, info in results.items():
            ...
            for zone, aus in self.facial_zones.items():
                ...
                # Special handling for midface zone - use the combined score approach
                if zone == 'mid':
                    # Process left side
                    left_confidence = self._calculate_confidence_score('left', zone, left_values, right_values, {})
                    left_processed = self.process_midface_combined_score(
                        info, zone, aus, 'left',
                        left_values, right_values,
                        left_values_normalized, right_values_normalized,
                        left_confidence, zone_paralysis, affected_aus_by_zone_side
                    )

                    # Process right side
                    right_confidence = self._calculate_confidence_score('right', zone, right_values, left_values, {})
                    right_processed = self.process_midface_combined_score(
                        info, zone, aus, 'right',
                        right_values, left_values,
                        right_values_normalized, left_values_normalized,
                        right_confidence, zone_paralysis, affected_aus_by_zone_side
                    )

                    # Skip standard processing for midface if we successfully processed with combined approach
                    if left_processed and right_processed:
                        continue

                # For lower face, try the combined score approach first
                if zone == 'lower':
                    # Process left side
                    left_confidence = self._calculate_confidence_score('left', zone, left_values, right_values, {})
                    left_processed = self.process_lower_face_combined_score(
                        info, zone, aus, 'left',
                        left_values, right_values,
                        left_values_normalized, right_values_normalized,
                        left_confidence, zone_paralysis, affected_aus_by_zone_side,
                        {}, self.asymmetry_thresholds[zone], self.confidence_thresholds[zone]
                    )

                    # Process right side
                    right_confidence = self._calculate_confidence_score('right', zone, right_values, left_values, {})
                    right_processed = self.process_lower_face_combined_score(
                        info, zone, aus, 'right',
                        right_values, left_values,
                        right_values_normalized, left_values_normalized,
                        right_confidence, zone_paralysis, affected_aus_by_zone_side,
                    ...
                    # Skip standard processing for lower face if we successfully processed with combined approach
                    if left_processed and right_processed:
                        continue

                # First, check if extreme asymmetry exists
