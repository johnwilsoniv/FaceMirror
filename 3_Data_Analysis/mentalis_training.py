# mentalis_training.py
# Trains an XGBoost model for Mentalis Synkinesis detection.
...
# Import specific components for Mentalis Synkinesis
try:
    from mentalis_features import prepare_data
    from mentalis_config import TRAINING_CONFIG, MODEL_FILENAMES, LOG_DIR, LOGGING_CONFIG, CLASS_NAMES
except ImportError:
    logging.error("Failed import from mentalis_features or mentalis_config.")
    raise SystemExit("Cannot proceed.")

# Configure logging
os.makedirs(LOG_DIR, exist_ok=True)
log_file = os.path.join(LOG_DIR, 'mentalis_training.log') # Specific log file
logging.basicConfig(
...
def train_and_evaluate():
    """ Trains and evaluates the Mentalis Synkinesis detection model using XGBoost. """
    logger.info("--- Starting Mentalis Synkinesis Detection Model Training (XGBoost) ---")
    ...
    try:
        ...
        if len(unique_targets) < 2: logger.warning("Only one class present in target data.")
    ...
    if stratify_param is None: logger.warning("Cannot stratify split with only one class.")
    ...
    try:
        ...
        logger.info(f"Training targets (before SMOTE/Weighting): { {CLASS_NAMES.get(k, k): v for k, v in train_target_counts.items()} }")
    ...
    if smote_enabled and scale_pos_weight is not None: logger.warning("SMOTE and scale_pos_weight configured. Prioritizing SMOTE.")

    if smote_enabled:
        if len(train_target_counts) < 2: logger.warning("Cannot SMOTE with one class. Training original.")
        elif min_class_count_train <= 1 or smote_k_neighbors < 1: logger.warning(f"Cannot SMOTE (min_class={min_class_count_train}, k={smote_k_neighbors}). Training original.")
        else:
            ...
            except Exception as smote_e: logger.error(f"SMOTE error: {smote_e}. Training original.", exc_info=True); X_train_scaled = scaler.fit_transform(X_train_df); X_train_fit, y_train_fit = X_train_scaled, y_train
    if not smote_enabled or ('X_train_scaled' not in locals()): # Ensure scaling happens if SMOTE fails/disabled
         logger.info("Scaling original training data (SMOTE disabled or failed).")
    ...
    if scale_pos_weight is not None and smote_enabled: logger.warning("Ignoring scale_pos_weight."); del xgb_params['scale_pos_weight']
    elif scale_pos_weight is not None: logger.info(f"Using scale_pos_weight={scale_pos_weight}.")
    ...
    try:
        if hasattr(base_model, 'feature_importances_'):
            ...
            else: logger.warning(f"MentS Imp ({len(imps)}) / Feat ({len(feature_names)}) length mismatch.")
    ...
    try:
        ...
        if not all([model_p, scaler_p, imp_p, feat_p]): raise ValueError("MentS Artifact paths missing.")
    ...
    logger.info("--- Mentalis Synkinesis Detection Model Training Finished ---")
