# upper_face_performance_analysis.py (Include Patient ID in Errors)
...
# --- Import necessary components for UPPER FACE ---
from upper_face_detector import UpperFaceParalysisDetector
try:
    from upper_face_features import extract_features_for_detection # Keep for potential future debug
except ImportError:
    logging.error("Could not import extract_features_for_detection.")
    extract_features_for_detection = None

try:
    from upper_face_config import LOG_DIR, CLASS_NAMES, FEATURE_CONFIG, UPPER_FACE_ACTIONS, UPPER_FACE_AUS
except ImportError:
    logging.warning("Could not import from upper_face_config. Using fallback definitions.")
    LOG_DIR = 'logs'; CLASS_NAMES = {0: 'None', 1: 'Partial', 2: 'Complete'}
    FEATURE_CONFIG = {'actions': ['RE'], 'aus': ['AU01_r', 'AU02_r'], 'min_value': 0.0001}
    UPPER_FACE_ACTIONS = ['RE']; UPPER_FACE_AUS = ['AU01_r', 'AU02_r']
# --- End Imports ---
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join('analysis_results', 'upperface_analysis.log'), mode='w'),
        logging.StreamHandler()
    ],
    force=True )
logger = logging.getLogger(__name__)

# --- Initialize the UPPER FACE Detector ---
try:
    upper_face_detector = UpperFaceParalysisDetector()
    logger.info("Upper Face Detector initialized successfully.")
except Exception as e:
    logger.error(f"Failed to initialize UpperFaceParalysisDetector: {e}", exc_info=True)
    upper_face_detector = None
# --- End Detector Init ---


def analyze_performance(results_file='combined_results.csv', expert_file='FPRS FP Key.csv',
                      output_dir='analysis_results'):
    """ Analyze UPPER FACE model performance by re-running detection. """
    os.makedirs(output_dir, exist_ok=True)
    logger.info(f"Starting UPPER FACE performance analysis by re-running detection")

    if upper_face_detector is None: logger.error("Upper face detector not available."); return
    ...
    try:
        ...
        expert_cols_to_standardize = ['Expert_Left_Upper_Face', 'Expert_Right_Upper_Face']
        ...
        # --- Re-run Upper Face Detection ---
        logger.info("Re-running upper face detection using loaded models...")
        predictions_left = []; predictions_right = []

        # --- DEBUG Control ---
        # first_patient_id = merged_df.iloc[0]['Patient ID'] if not merged_df.empty else None
        # logger.info(f"DEBUG: First Patient ID: {first_patient_id}")
        # debug_done = False
        # --- END DEBUG ---
        ...
        for index, row in merged_df.iterrows():
            ...
            # if not debug_done and current_patient_id == first_patient_id and extract_features_for_detection is not None:
            #     try:
            #         debug_features_left = extract_features_for_detection(row_data_dict, 'Left', 'upper')
            #         logger.info(f"DEBUG (analysis): Upper Face Features for Patient {first_patient_id} Left:\n{debug_features_left}")
            #     except Exception as debug_e: logger.error(f"DEBUG: Error generating features: {debug_e}", exc_info=True)
            #     # logger.info("DEBUG: Exiting after checking first patient.")
            #     # sys.exit()
            #     debug_done = True
            # --- END DEBUG ---

            # Predict Left
            try: result_l, _, _ = upper_face_detector.detect(row_data_dict, 'Left', 'upper'); predictions_left.append(result_l)
            except Exception as e_l: logger.error(f"Error predict left upper {current_patient_id}: {e_l}"); predictions_left.append('Error')
            # Predict Right
            try: result_r, _, _ = upper_face_detector.detect(row_data_dict, 'Right', 'upper'); predictions_right.append(result_r)
            except Exception as e_r: logger.error(f"Error predict right upper {current_patient_id}: {e_r}"); predictions_right.append('Error')

        merged_df['ML_Pred_Left_Upper'] = predictions_left
        merged_df['ML_Pred_Right_Upper'] = predictions_right

        # Analyze upper face paralysis
        analyze_zone(merged_df, 'Upper Face', 'ML_Pred_Left_Upper', 'Expert_Left_Upper_Face', 'ML_Pred_Right_Upper', 'Expert_Right_Upper_Face', output_dir)
...
def analyze_zone(data, zone_name, pred_left_col, expert_left_col, pred_right_col, expert_right_col, output_dir):
    ...
    required_cols = ['Patient ID', pred_left_col, expert_left_col, pred_right_col, expert_right_col] # Add Patient ID
    ...
    # --- MODIFICATION: Include 'Patient ID' ---
    ...
    # --- MODIFICATION: Adjust column renaming ---
    ...
    logger.info(f"Sample data for {zone_name} analysis:\n{zone_df.head().to_string()}") # Will now show Patient ID

    try:
        zone_df_valid = zone_df[zone_df['Prediction'] != 'Error'].copy() # zone_df_valid now has Patient ID
        ...
        # Generate CMs (logic remains the same)
        ...
        # Pass valid data (with Patient ID) to error analysis functions
        perform_error_analysis(zone_df_valid, output_dir, f"{zone_name}_errors_ML_Preds")
        analyze_critical_errors(zone_df_valid, output_dir, f"{zone_name}_critical_errors")
        analyze_partial_errors(zone_df_valid, output_dir, f"{zone_name}_partial_errors")
...
def visualize_confusion_matrix(cm, categories, title, output_dir):
    # (Keep this function as before)
    try:
        plt.figure(figsize=(8, 6)); sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=categories, yticklabels=categories, cbar=False)
        plt.xlabel('Predicted Label'); plt.ylabel('True Label'); plt.title(f"{title} Confusion Matrix")
        save_path = os.path.join(output_dir, f"{title.replace(' ', '_').replace('(','').replace(')','')}_confusion_matrix.png")
        plt.tight_layout(); plt.savefig(save_path); logger.info(f"CM saved: {save_path}"); plt.close()
...
def perform_error_analysis(data, output_dir, filename):
    ...
    try:
        with open(path, 'w') as f:
            ...
            error_cases_reset = error_cases.reset_index() # Use reset index for sequential logging index
