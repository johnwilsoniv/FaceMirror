# upper_face_detector.py (Refactored with DataFrame Scaling)
...
try:
    from upper_face_config import ( # Import from upper_face config
    ...
    MODEL_FILENAMES = {'base_model': os.path.join(MODEL_DIR, 'upper_face_model.pkl'),
...
try:
    feature_list_path_check = MODEL_FILENAMES.get('feature_list', os.path.join(MODEL_DIR, 'upper_face_features.list'))
    ...
    from upper_face_features import extract_features_for_detection # Import from upper_face features
except ImportError:
    logging.error("Could not import extract_features_for_detection from upper_face_features.py.")
    extract_features_for_detection = None

# Configure logging
logger = logging.getLogger(__name__) # Assuming configured by main

class UpperFaceParalysisDetector: # Renamed class
    """ Detects upper face paralysis using ML model. """

    def __init__(self):
        logger.info("Initializing UpperFaceParalysisDetector")
        self.base_model = None; self.base_scaler = None; self.feature_names = None
        self._load_models()

    def _load_models(self):
        """Load the base model, scaler, and feature names."""
        # ... (Identical loading logic as lower_face_detector) ...
        try:
            ...
            if not all([model_path, scaler_path, features_path]): logger.error("Missing upper-face config paths."); return
            ...
            if paths_exist:
                ...
                logger.info("Upper face artifacts loaded."); assert isinstance(self.feature_names, list), "Feature names not a list"
            ...
            else: logger.error("Upper face artifacts missing."); self.base_model, self.base_scaler, self.feature_names = None, None, None
    ...
    def detect(self, row_data, side, zone):
        """ Detect using base model prediction. Accepts row data. """
        if not all([self.base_model, self.base_scaler, self.feature_names]):
            logger.warning(f"Upper Face components unavailable for {side} {zone}.")
            return 'Error', 0.0, {'error': 'Model/Scaler/Features missing'}
        if extract_features_for_detection is None:
             logger.error("Upper face feature extraction unavailable."); return 'Error', 0.0, {'error': 'Feature func missing'}

        try:
            features_list = extract_features_for_detection(row_data, side, zone)
            if features_list is None: logger.error(f"U-Feat extract fail {side} {zone}"); return 'Error', 0.0, {'error': 'Feat extract failed'}
            if len(features_list) != len(self.feature_names): logger.error(f"U-Feat mismatch {side} {zone}"); return 'Error', 0.0, {'error': 'Feat mismatch'}
            ...
            logger.debug(f"Upper Face Detect - {side} {zone}: Result={result}, Conf={confidence:.3f}")
            return result, confidence, details

        except Exception as e:
            logger.error(f"Error in upper face detect {side} {zone}: {e}", exc_info=True)
