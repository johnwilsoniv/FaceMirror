# brow_cocked_performance_analysis.py
# Analyzes performance of the Brow Cocked detector.
# Includes threshold evaluation functionality.
# Corrected expert key handling in analyze_performance.
...
try:
    from brow_cocked_detector import BrowCockedDetector
    from brow_cocked_config import LOG_DIR, CLASS_NAMES, MODEL_FILENAMES, DETECTION_THRESHOLD
    from brow_cocked_features import extract_features_for_detection # Keep for potential direct use/debug
except ImportError:
    logging.error("Could not import BrowCockedDetector or config.")
...
log_file = os.path.join(ANALYSIS_OUTPUT_DIR, 'brow_cocked_analysis_thresh.log')
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
                    handlers=[logging.FileHandler(log_file, mode='w'), logging.StreamHandler()], force=True)
logger = logging.getLogger(__name__)

# Initialize the Detector
brow_cocked_detector = None
try:
    if not all(os.path.exists(MODEL_FILENAMES.get(k)) for k in ['model', 'scaler', 'feature_list']):
        logger.error(f"Brow Cocked model artifacts missing. Cannot run analysis.")
    else:
        brow_cocked_detector = BrowCockedDetector()
        if brow_cocked_detector.model is None: raise RuntimeError("Detector failed to load artifacts.")
        logger.info("BrowCockedDetector initialized for analysis.")
except Exception as e: logger.error(f"Failed to initialize BrowCockedDetector: {e}", exc_info=True); brow_cocked_detector = None
...
# --- Generic Analysis Function (analyze_synkinesis_results - UNCHANGED) ---
def analyze_synkinesis_results(data, synk_type_name, pred_left_col, expert_left_col, pred_right_col, expert_right_col, output_dir):
    logger.info(f"--- Analyzing {synk_type_name} Performance (at Detector Threshold: {brow_cocked_detector.threshold if brow_cocked_detector else 'N/A'}) ---")
    ...
    try:
        ...
        thresh_str = str(brow_cocked_detector.threshold).replace('.', '_') if brow_cocked_detector else 'NA'
...
# --- visualize_confusion_matrix (UNCHANGED) ---
...
# --- perform_error_analysis (UNCHANGED) ---
...
# --- NEW Function: Evaluate Thresholds (UNCHANGED) ---
def evaluate_thresholds(data, proba_left_col, expert_left_col, proba_right_col, expert_right_col, output_dir):
    logger.info(f"\n--- Evaluating Brow Cocked Performance Across Thresholds ---")
    required_cols = [proba_left_col, expert_left_col, proba_right_col, expert_right_col]
    if not all(col in data.columns for col in required_cols): logger.error(f"Missing columns for threshold eval: {[c for c in required_cols if c not in data.columns]}. Aborting."); return
    ...
    results_df = pd.DataFrame(results); save_path = os.path.join(output_dir, "brow_cocked_threshold_evaluation.csv")
    ...
    try:
        ...
        plt.xlabel("Recall (Brow Cocked)"); plt.ylabel("Precision (Brow Cocked)"); plt.title("Brow Cocked Precision-Recall Curve vs. Threshold"); plt.grid(True)
        plt.xlim([0.0, 1.05]); plt.ylim([0.0, 1.05])
        for i, row in results_df.iterrows():
             if i % 2 == 0: plt.text(row['Recall'], row['Precision'] + 0.02, f"{row['Threshold']:.2f}")
        pr_curve_path = os.path.join(output_dir, "brow_cocked_precision_recall_curve.png")
...
# --- Main Analysis Function (Modified Expert Key Handling) ---
def analyze_performance(results_file='combined_results.csv', expert_file='FPRS FP Key.csv',
                      output_dir=ANALYSIS_OUTPUT_DIR):
    """ Analyze BROW COCKED model performance by re-running detection,
        reporting at default threshold, and evaluating across thresholds. """
    os.makedirs(output_dir, exist_ok=True); logger.info(f"--- Starting BROW COCKED Performance Analysis ---")
    current_threshold = brow_cocked_detector.threshold if brow_cocked_detector else DETECTION_THRESHOLD
    logger.info(f"Using Detection Threshold: {current_threshold}")
    logger.info(f"Files: {results_file}, {expert_file}. Output: {output_dir}")
    if brow_cocked_detector is None: logger.error("Detector unavailable."); return

    try:
        logger.info(f"Loading data..."); results_df = pd.read_csv(results_file, low_memory=False); expert_df = pd.read_csv(expert_file)

        # --- Robust Expert Key Handling ---
        # Define original and target names
        orig_left_col = 'Cocked Left'
        orig_right_col = 'Cocked Right'
        target_left_col = 'Expert_Left_BrowCocked'
        target_right_col = 'Expert_Right_BrowCocked'

        # Check if original columns exist
        if orig_left_col not in expert_df.columns:
            logger.error(f"Expert key file missing required column: '{orig_left_col}'")
            return
        if orig_right_col not in expert_df.columns:
            logger.error(f"Expert key file missing required column: '{orig_right_col}'")
            return

        # Rename the specific columns we need
        expert_df = expert_df.rename(columns={
            orig_left_col: target_left_col,
            orig_right_col: target_right_col
        })
        # Also handle Patient ID renaming separately if needed
        if 'Patient' in expert_df.columns and 'Patient ID' not in expert_df.columns:
             expert_df = expert_df.rename(columns={'Patient': 'Patient ID'})
        elif 'Patient ID' not in expert_df.columns:
             logger.error("Expert key file missing 'Patient ID' column.")
             return

        # Process targets using the new column names
        expert_df[target_left_col] = process_targets(expert_df[target_left_col]) # Use direct access now
        expert_df[target_right_col] = process_targets(expert_df[target_right_col]) # Use direct access now
        logger.info("Expert labels standardized.")
        # --- End Robust Expert Key Handling ---
        ...
        expert_cols = ['Patient ID', target_left_col, target_right_col]
        ...
        # Check targets exist after merge (should always be true now)
        if target_left_col not in merged_df.columns or target_right_col not in merged_df.columns:
             logger.error(f"Target columns '{target_left_col}' or '{target_right_col}' missing after merge."); return


        logger.info("Re-running Brow Cocked detection (getting predictions and probabilities)...");
        predictions_left, predictions_right = [], []; confidences_left, confidences_right = [], []
        probabilities_left, probabilities_right = [], []
        ...
        for index, row in merged_df.iterrows():
            ...
            try:
                ...
                if features_list_l is None: raise ValueError("Feature extraction failed L")
                if len(features_list_l) != len(brow_cocked_detector.feature_names): raise ValueError(f"Feature mismatch L: Got {len(features_list_l)}, Exp {len(brow_cocked_detector.feature_names)}")
                features_df_l = pd.DataFrame([features_list_l], columns=brow_cocked_detector.feature_names); features_df_l = features_df_l.fillna(0).replace([np.inf, -np.inf], 0)
                scaled_features_l = brow_cocked_detector.scaler.transform(features_df_l); scaled_features_l = np.nan_to_num(scaled_features_l, nan=0.0, posinf=0.0, neginf=0.0)
                proba_array_l = brow_cocked_detector.model.predict_proba(scaled_features_l)[0]
                if len(proba_array_l) < 2: raise ValueError("Prob array shape error L")
                proba_pos_l = proba_array_l[1]; pred_l = 1 if proba_pos_l >= brow_cocked_detector.threshold else 0; conf_l = proba_array_l[pred_l]
            ...
            try:
                ...
                if features_list_r is None: raise ValueError("Feature extraction failed R")
                if len(features_list_r) != len(brow_cocked_detector.feature_names): raise ValueError(f"Feature mismatch R: Got {len(features_list_r)}, Exp {len(brow_cocked_detector.feature_names)}")
                features_df_r = pd.DataFrame([features_list_r], columns=brow_cocked_detector.feature_names); features_df_r = features_df_r.fillna(0).replace([np.inf, -np.inf], 0)
                scaled_features_r = brow_cocked_detector.scaler.transform(features_df_r); scaled_features_r = np.nan_to_num(scaled_features_r, nan=0.0, posinf=0.0, neginf=0.0)
                proba_array_r = brow_cocked_detector.model.predict_proba(scaled_features_r)[0]
                if len(proba_array_r) < 2: raise ValueError("Prob array shape error R")
                proba_pos_r = proba_array_r[1]; pred_r = 1 if proba_pos_r >= brow_cocked_detector.threshold else 0; conf_r = proba_array_r[pred_r]
        ...
        merged_df['ML_Pred_Left_BrowCocked'] = predictions_left; merged_df['ML_Conf_Left_BrowCocked'] = confidences_left; merged_df['ML_Proba_Left_BrowCocked'] = probabilities_left
        merged_df['ML_Pred_Right_BrowCocked'] = predictions_right; merged_df['ML_Conf_Right_BrowCocked'] = confidences_right; merged_df['ML_Proba_Right_BrowCocked'] = probabilities_right
        analysis_df = merged_df[ (merged_df['ML_Pred_Left_BrowCocked'] != -1) & (merged_df['ML_Pred_Right_BrowCocked'] != -1) &
                                 (merged_df['ML_Proba_Left_BrowCocked'].notna()) & (merged_df['ML_Proba_Right_BrowCocked'].notna()) ].copy()
        ...
        analyze_synkinesis_results(analysis_df, 'Brow Cocked',
                                   'ML_Pred_Left_BrowCocked', target_left_col, # Use target_left_col
                                   'ML_Pred_Right_BrowCocked', target_right_col, # Use target_right_col
                                   output_dir)

        # --- Evaluate Across Thresholds ---
        evaluate_thresholds(
            data=analysis_df,
            proba_left_col='ML_Proba_Left_BrowCocked', expert_left_col=target_left_col, # Use target_left_col
            proba_right_col='ML_Proba_Right_BrowCocked', expert_right_col=target_right_col, # Use target_right_col
            output_dir=output_dir
        )

        logger.info("--- BROW COCKED Performance Analysis Complete ---")
    except Exception as e: logger.error(f"Unexpected error: {str(e)}", exc_info=True)


if __name__ == "__main__":
    logger.info("Running Brow Cocked performance analysis script directly.")
    analyze_performance()
