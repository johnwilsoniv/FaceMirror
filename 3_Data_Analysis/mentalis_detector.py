...
# Detects Mentalis Synkinesis using a trained ML model and adjustable threshold.
...
try:
    ...
    from mentalis_config import MODEL_FILENAMES, CLASS_NAMES, LOG_DIR, LOGGING_CONFIG, DETECTION_THRESHOLD
...
class MentalisDetector:
    ...
    def _load_models(self):
        ...
        try:
            ...
            if not all(os.path.exists(p) for p in [model_p, scaler_p, feat_p] if p):
                 ...
                 logger.error(f"Mentalis artifact(s) missing: {missing}"); return
    ...
    def detect_mentalis_synkinesis(self, row_data, side):
        """
        Detects Mentalis Synkinesis using ML model and adjustable threshold.
        ...
        try:
            ...
            if len(probabilities) < 2:
                logger.error(f"MentS ({side_label}): predict_proba returned unexpected shape: {probabilities.shape}. Cannot determine positive class probability.")
                return False, 0.0

            # Assuming Class 1 ('Mentalis Synkinesis') is the positive class (index 1)
            ...
            logger.debug(f"ML Mentalis Detect {side_label}: Prob(Synk)={positive_class_proba:.3f}, Threshold={self.threshold}, Pred={predicted_class_index} ({predicted_label}), Conf={confidence:.3f}")
