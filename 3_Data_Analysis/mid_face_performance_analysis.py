# mid_face_performance_analysis.py (Include Patient ID in Errors)
...
# --- Import necessary components for MID FACE ---
from mid_face_detector import MidFaceParalysisDetector
try:
    from mid_face_features import extract_features_for_detection
except ImportError:
    logging.error("Could not import extract_features_for_detection for debugging."); extract_features_for_detection = None
try:
    from mid_face_config import LOG_DIR, CLASS_NAMES
except ImportError:
    logging.warning("Could not import from mid_face_config."); LOG_DIR = 'logs'; CLASS_NAMES = {0: 'None', 1: 'Partial', 2: 'Complete'}
# --- End Imports ---

# Configure logging
os.makedirs('analysis_results', exist_ok=True)
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', handlers=[logging.FileHandler(os.path.join('analysis_results', 'midface_analysis.log'), mode='w'), logging.StreamHandler()], force=True)
logger = logging.getLogger(__name__)

# --- Initialize the MID FACE Detector ---
try:
    mid_face_detector = MidFaceParalysisDetector(); logger.info("Mid Face Detector initialized successfully.")
except Exception as e: logger.error(f"Failed to initialize MidFaceParalysisDetector: {e}", exc_info=True); mid_face_detector = None
# --- End Detector Init ---


def analyze_performance(results_file='combined_results.csv', expert_file='FPRS FP Key.csv',
                      output_dir='analysis_results'):
    """ Analyze MID FACE model performance by re-running detection. """
    os.makedirs(output_dir, exist_ok=True)
    logger.info(f"Starting MID FACE performance analysis by re-running detection")

    if mid_face_detector is None: logger.error("Mid face detector not available."); return
    ...
    try:
        ...
        expert_cols_to_standardize = ['Expert_Left_Mid_Face', 'Expert_Right_Mid_Face']
        ...
        logger.info("Re-running mid face detection using loaded models...")
        ...
        for index, row in merged_df.iterrows():
            ...
            #     try:
            #         debug_features_left = extract_features_for_detection(row_data_dict, 'Left', 'mid')
            #         logger.info(f"DEBUG (analysis): Mid Face Features for Patient {first_patient_id} Left:\n{debug_features_left}")
            ...
            try: result_l, _, _ = mid_face_detector.detect(row_data_dict, 'Left', 'mid'); predictions_left.append(result_l)
            except Exception as e_l: logger.error(f"Error predict left mid {current_patient_id}: {e_l}"); predictions_left.append('Error')
            try: result_r, _, _ = mid_face_detector.detect(row_data_dict, 'Right', 'mid'); predictions_right.append(result_r)
            except Exception as e_r: logger.error(f"Error predict right mid {current_patient_id}: {e_r}"); predictions_right.append('Error')

        merged_df['ML_Pred_Left_Mid'] = predictions_left
        merged_df['ML_Pred_Right_Mid'] = predictions_right

        analyze_zone(merged_df, 'Mid Face', 'ML_Pred_Left_Mid', 'Expert_Left_Mid_Face', 'ML_Pred_Right_Mid', 'Expert_Right_Mid_Face', output_dir)
...
def analyze_zone(data, zone_name, pred_left_col, expert_left_col, pred_right_col, expert_right_col, output_dir):
    ...
    try:
        ...
        analyze_critical_errors(zone_df_valid, output_dir, f"midface_critical_errors") # Keep consistent filename
        analyze_partial_errors(zone_df_valid, output_dir, f"midface_partial_errors") # Keep consistent filename
