# snarl_smile_performance_analysis.py (Updated Import/Instantiation)
...
try:
    ...
    from snarl_smile_config import LOG_DIR, CLASS_NAMES
except ImportError:
...
try:
    ...
    if snarl_smile_detector.model is None: raise RuntimeError("Detector init failed: model not loaded.")
    logger.info("SnarlSmileDetector initialized for analysis.")
except Exception as e:
    logger.error(f"Failed to initialize SnarlSmileDetector: {e}", exc_info=True); snarl_smile_detector = None
...
def analyze_performance(results_file='combined_results.csv', expert_file='FPRS FP Key.csv',
    ...
    os.makedirs(output_dir, exist_ok=True); logger.info(f"--- Starting SNARL-SMILE Performance Analysis ---"); logger.info(f"Files: {results_file}, {expert_file}. Output: {output_dir}")
    ...
    try:
        ...
        expert_cols = ['Patient ID', 'Expert_Left_Snarl_Smile', 'Expert_Right_Snarl_Smile']
        merged_df = pd.merge(results_df, expert_df[expert_cols], on='Patient ID', how='inner', validate="many_to_one")
        logger.info(f"Merged data: {len(merged_df)} patients with Snarl-Smile labels.")
        if merged_df.empty: logger.error("Merge failed."); return
        logger.info("Re-running Snarl-Smile detection..."); predictions_left, predictions_right = [], []; confidences_left, confidences_right = [], []
        ...
        analyze_synkinesis_results(analysis_df, 'Snarl-Smile', 'ML_Pred_Left_Snarl_Smile', 'Expert_Left_Snarl_Smile', 'ML_Pred_Right_Snarl_Smile', 'Expert_Right_Snarl_Smile', output_dir)
...
def process_targets(target_series):
    ...
    mapped = s_clean.map(mapping); final_mapped = mapped.fillna(0)
