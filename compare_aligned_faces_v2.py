#!/usr/bin/env python3
"""Compare Python vs C++ aligned faces - using C++ landmarks."""
...
print(f"C++ landmarks shape: {landmarks.shape}")
print(f"  Landmark 0 (jaw left): {landmarks[0]}")
print(f"  Landmark 8 (chin): {landmarks[8]}")
print(f"  Landmark 30 (nose tip): {landmarks[30]}")

# Estimate pose from landmarks
# The pose_tx, pose_ty represent the face center in image coordinates
# Use the nose tip (landmark 30) as an approximation
# Actually, OpenFace uses the mean of certain landmarks + PDM mean
# For now, use the center of all landmarks as a reasonable approximation
face_center = landmarks.mean(axis=0)
pose_tx = face_center[0]
pose_ty = face_center[1]
print(f"Estimated Pose (face center): tx={pose_tx:.2f}, ty={pose_ty:.2f}")
