#!/usr/bin/env python3
"""
MTL Model Architecture Analysis for Apple Neural Engine Compatibility

This script analyzes the MTL EfficientNet-B0 model to identify:
...
def analyze_model_structure(model):
    ...
    print("=" * 80)
    print("MTL MODEL ARCHITECTURE ANALYSIS")
    print("=" * 80)
    print()
    ...
    for name, module in model.named_modules():
        ...
        if any(act in module_type for act in ['ReLU', 'Sigmoid', 'Tanh', 'GELU', 'SiLU', 'Swish', 'Hardswish']):
            ...
            elif 'Hardswish' in module_type:
                analysis['activation_functions']['Hardswish'].append(name)
                # Hardswish is used in EfficientNet and is reasonably ANE-compatible
                # It's x * ReLU6(x + 3) / 6, which can be fused
                layer_info['issue'] = 'Hardswish - acceptable for ANE (commonly used in EfficientNet)'
...
def print_analysis(analysis):
    ...
    if analysis['activation_functions']:
        for act_type, layers in analysis['activation_functions'].items():
            if act_type in ['ReLU', 'Sigmoid', 'Tanh']:
                ...
                ane_status = "~ Acceptable (used in EfficientNet)"
            else:
    ...
    if not analysis['problematic_layers']:
        ...
        print("     - Use fixed input shapes (224x224)")
        print("     - Use channels-last memory format (NHWC)")
        print("  3. This should achieve 90%+ ANE coverage without model changes")
        print()
        print("Expected timeline: 2-4 hours (vs 2-3 weeks for architecture changes)")
    else:
        ...
        print("  - Hardswish (if present) is acceptable for EfficientNet")
        print("  - Timeline: 2-4 hours")
        print("  - Expected ANE coverage: 85-95%")
        print()
        ...
        print()
        print("Recommendation: Try Approach A first (like we did with STAR)")
...
def main():
    ...
    print()
    print("Loading MTL model from OpenFace...")
    print()
    ...
    report_path = script_dir / 'mtl_architecture_analysis_report.txt'
