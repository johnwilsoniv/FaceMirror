...
# === DIAGNOSTIC SUMMARY ===
print("=" * 70)
print("WHISPER HANDLER INITIALIZATION")
print("=" * 70)
print(f"MLX Whisper Available: {MLX_WHISPER_AVAILABLE}")
print(f"Silero VAD Available: {SILERO_VAD_AVAILABLE}")
print(f"Faster-Whisper Available: {FASTER_WHISPER_AVAILABLE}")
print(f"WhisperX Alignment Available: {WHISPERX_ALIGN_AVAILABLE}")
print(f"PyTorch Available: {TORCH_AVAILABLE}")
print(f"Is Apple Silicon: {IS_APPLE_SILICON}")
if MLX_WHISPER_AVAILABLE and SILERO_VAD_AVAILABLE:
    print(">>> WILL USE: MLX Whisper + Silero VAD (3-6x faster) <<<")
...
def check_mlx_model_cached(model_name):
    """
    Check if MLX Whisper model is cached locally.

    Args:
        model_name: Model name (e.g., "mlx-community/whisper-large-v3-mlx")
    ...
    try:
        ...
        # MLX models are stored in HuggingFace cache
        cache_dir = Path.home() / ".cache" / "huggingface" / "hub"

        # Convert model name to cache directory format
        # e.g., "mlx-community/whisper-large-v3-mlx" -> "models--mlx-community--whisper-large-v3-mlx"
        model_cache_name = "models--" + model_name.replace("/", "--")
        model_cache_path = cache_dir / model_cache_name

        if model_cache_path.exists():
            # Check if it has the essential model files
        ...
        print(f"Warning: Could not check MLX model cache: {e}")
        return False
...
def transcribe_with_mlx(audio_path, model_name, speech_segments, language="en", beam_size=5, progress_callback=None, cancellation_check=None):
    ...
    Args:
        ...
        cancellation_check: Optional callback function() -> bool that returns True if processing should be cancelled
    ...
    model_is_cached = check_mlx_model_cached(model_name)
    ...
    for idx, seg in enumerate(speech_segments):
        ...
        if cancellation_check and cancellation_check():
            print(f"MLX Whisper: Cancellation detected at segment {idx+1}/{len(speech_segments)}")
            raise InterruptedError("MLX transcription cancelled by user")

        start_time = seg['start']
        end_time = seg['end']

        # Show appropriate progress message
        if idx == 0 and not model_is_cached:
            msg = f"Downloading & transcribing segment 1/{len(speech_segments)}..."
            if progress_callback:
                progress_callback(50, msg)
            print(f"MLX Whisper: {msg}")
        else:
            progress_pct = 50 + int((idx / len(speech_segments)) * 40)  # 50-90% range
            msg = f"Processing segment {idx+1}/{len(speech_segments)}: {start_time:.2f}s - {end_time:.2f}s"
            if progress_callback:
...
class WhisperHandler(QThread):
    ...
    def __init__(self, audio_path, temp_dir_for_cleanup, vad_parameters, model_name="large-v3", parent=None, debug_keep_audio=False, preloaded_model=None, skip_alignment=False):
        ...
        print("=" * 70)
        if self.use_mlx:
            print(">>> CONFIGURED TO USE: MLX Whisper + Silero VAD <<<")
            print("  - Apple Silicon optimized")
            print("  - 3-6x faster than faster-whisper")
            print("  - Models load on-demand (no preloading)")
        else:
            ...
            print(">>> CONFIGURED TO USE: Faster-Whisper <<<")
            if not MLX_WHISPER_AVAILABLE:
                print("  Reason: MLX Whisper not available")
            if not SILERO_VAD_AVAILABLE:
                print("  Reason: Silero VAD not available")
            if preloaded_model:
                print("  Using pre-loaded model from splash screen")
        print("=" * 70)
    ...
    def run(self):
        ...
        try:
            ...
            self.emit_progress(10, f"Setting up transcription...")

            # Initialize device variable (will be set properly in each branch)
            device = "cpu"
            ...
            if cached_segments:
                ...
                if not speech_segments:
                    ...
                    self.emit_progress(40, f"Preparing MLX Whisper transcription...")
                    faster_whisper_segments = transcribe_with_mlx(
                        ...
                        progress_callback=self.emit_progress,  # Pass progress callback for GUI updates
                        cancellation_check=lambda: self._is_cancelled  # Allow cancellation between segments
                    )
                ...
                # Set up device and optimization parameters for faster-whisper
                ...
                if device == "cuda":
                    ...
                    compute_type = "int8"
                    print("FasterWhisper: Running on CPU with int8 quantization")
            ...
            # Don't emit processing_finished when cancelled - the controller is already
            # loading a new video and doesn't want stale/empty results
            print(f"FasterWhisper Thread: Not emitting results (cancelled by user)")
