...
class ApplicationController(QObject):
    def __init__(self, app_instance, whisper_model=None):
        ...
        self.playing_range_start_frame = None  # Start frame of currently playing range
        self.playing_range_end_frame = None    # End frame of currently playing range
        self.playing_range_index = None        # Index of currently playing range for visual feedback
        # --- End Range Playback State ---
        self.next_timeline_event_index = 0
        self.selected_timeline_range_data = None
        self.current_selected_index = -1

        # --- Performance Optimization: Action Range Index for Fast Lookups ---
        # Index for O(1) lookup of action ranges by (action_code, start_frame)
        # Rebuilt when action_ranges_changed signal fires
        self._action_range_index = {}
        # --- End Action Range Index ---

        # --- Performance Optimization: Timeline Update Throttling ---
        self._timeline_update_timer = QTimer()
        self._timeline_update_timer.setSingleShot(True)
        self._timeline_update_timer.setInterval(33)  # ~30 FPS max update rate
        self._timeline_update_timer.timeout.connect(self._do_timeline_update)
        self._timeline_update_pending = False
        self._last_ranges_snapshot = None  # Cache to avoid unnecessary updates
        # --- End Timeline Throttling ---
    ...
    def _run_startup_sequence(self): # (Unchanged)
        ...
        s1_dir = config_paths.get_s1_processed_dir()
        if s1_dir.exists():
            file_dialog.setDirectory(str(s1_dir))
        ...
        if file_dialog.exec_():
            ...
            # Check for existing outputs and get user confirmation (consolidated single dialog)
            if file_sets:
                ...
                if first_video:
                    ...
                    if processed_count == 0:
                        # No files have outputs - proceed without asking
                        print(f"No existing outputs found for {total} files, proceeding...")
                    elif unprocessed_count > 0:
                        # SCENARIO 1: Mixed - some processed, some not
                        # Build message with file examples
                        message = (
                            f"Found {processed_count} of {total} file(s) already processed.\n\n"
                            f"Files already processed:\n"
                        )
                        # Show up to 3 processed files as examples
                        for fs in output_check['processed'][:3]:
                            message += f"  • {os.path.basename(fs['video'])}\n"
                        if processed_count > 3:
                            message += f"  ... and {processed_count - 3} more\n"
                        ...
                        user_choice = native_dialogs.ask_three_choice(
                            ...
                            message,
                            f"Re-run All ({total})",
                            f"Process Unprocessed ({unprocessed_count})",
                            "Cancel",
                            default_button=2  # Default to "Process Unprocessed"
                        )

                        if user_choice == 1:
                            # Re-run All - keep all file_sets
                            print(f"User chose: Re-run All ({total} files)")
                        elif user_choice == 2:
                            # Process Unprocessed - keep only unprocessed
                            file_sets = output_check['unprocessed']
                            print(f"User chose: Process Unprocessed ({len(file_sets)} files)")
                        else:
                            # Cancel or None
                            print("User cancelled at existing outputs dialog")
                            return False
                    else:
                        # SCENARIO 2: All files already processed
                        # Build message with file examples
                        message = (
                            f"All {total} selected file(s) already have existing outputs.\n\n"
                            f"Files already processed:\n"
                        )
                        # Show up to 5 processed files as examples
                        for fs in output_check['processed'][:5]:
                            message += f"  • {os.path.basename(fs['video'])}\n"
                        if processed_count > 5:
                            message += f"  ... and {processed_count - 5} more\n"

                        message += "\n\nRe-process all files? (This will overwrite existing outputs)"

                        user_response = native_dialogs.ask_yes_no(
                            "Existing Outputs Detected",
                            message,
                            default_yes=False  # Default to No for safety
                        )

                        if not user_response:
                            print("User declined to re-process all files")
                            return False
                        else:
                            print(f"User chose: Re-process all {total} files")
    ...
    def _connect_signals(self): # (Unchanged)
        ...
        # Use DirectConnection for instant UI feedback (bypasses event queue)
        self.playback_manager.playback_state_changed.connect(
            self.ui_manager.set_play_button_state, Qt.DirectConnection)
        ...
        # Connect range playback boundary monitoring
        self.playback_manager.frame_update_needed.connect(self._check_range_playback_boundary)
        ...
        self.action_tracker.action_ranges_changed.connect(self._rebuild_action_range_index)
        if self.window.timeline_widget:
            ...
            self.window.timeline_widget.seek_requested.connect(self._handle_seek_request)
    ...
    def _toggle_playback(self, should_play): # (Unchanged)
        # Cancel any range playback when user manually controls playback
        self._stop_range_playback()
    ...
    def _handle_frame_update(self, frame_number, qimage): # (Unchanged)
        # === PERFORMANCE LOGGING: Track time spent in frame update handler ===
        ...
        # REMOVED: Unused QPixmap creation (was wasting CPU time)
        ...
        if self.playback_manager.is_playing:
            ...
            # Log if timeline trigger check takes >50ms
            if (_t7 - _t6) > 0.050:
                log_perf_warning(f"Timeline trigger check took {(_t7-_t6)*1000:.1f}ms at F{frame_number}")

        # Log total time if >16ms (60 FPS threshold)
        _total_time = time.time() - _start_time
        if _total_time > 0.016:
            log_perf_warning(f"Frame update F{frame_number} took {_total_time*1000:.1f}ms "
                  f"(UI:{(_t2-_t1)*1000:.1f}ms, GetAction:{(_t3-_t2)*1000:.1f}ms, "
                  f"Display:{(_t4-_t3)*1000:.1f}ms, Update:{(_t5-_t4)*1000:.1f}ms)")
        # === END PERFORMANCE LOGGING ===

    @pyqtSlot(int)
    def _handle_seek_request(self, frame_number):
        """Handle seek requests from timeline widget (e.g., arrow key navigation, mouse click)"""
        # === PERFORMANCE LOGGING: Track seek operation (user reports 2-second delay) ===
        _seek_start = time.time()

        # === OPTIMIZATION: Removed redundant pause ===
        # The seek() method already handles pausing internally if needed (qt_media_player.py:176-177)
        # Pausing here was causing 196ms delay + another pause inside seek()
        # OLD CODE:
        # if self.playback_manager.is_playing:
        #     self.playback_manager.pause()

        # Seek to requested frame (handles pausing internally)
        self.playback_manager.seek(frame_number)
        _t3 = time.time()

        # Log if seek took >100ms
        _total_seek_time = _t3 - _seek_start
        if _total_seek_time > 0.100:
            log_perf_warning(f"Seek to F{frame_number} took {_total_seek_time*1000:.1f}ms "
                           f"(direct seek, no pre-pause)")
        elif _total_seek_time > 0.050:
            log_perf_info(f"Seek to F{frame_number} took {_total_seek_time*1000:.1f}ms")
        # === END PERFORMANCE LOGGING ===
    ...
    # (_check_timeline_triggers, _reset_timeline_iterator, _handle_video_finished, _trigger_next_confirmation, _show_confirmation_ui_internal, _trigger_near_miss_prompt, _show_near_miss_ui_internal, _play_audio_segment, _cleanup_active_snippet, _handle_discard_confirmation_prompt, _clear_prompt_state_and_resume, _calculate_action_end_frame, _force_ui_update, _apply_action - unchanged)
    def _check_timeline_triggers(self, frame_number):
        """
        Check if current frame should trigger any timeline events (confirmations, near-miss prompts).
        OPTIMIZED: Uses action range index for O(1) lookups instead of O(n) linear searches.
        """
        ...
        while self.next_timeline_event_index < len(self.final_timeline_events):
            ...
            if frame_number >= trigger_frame:
                ...
                if event_to_trigger['type'] == 'command':
                     ...
                     # === OPTIMIZATION: Use index for O(1) lookup instead of O(n) linear search ===
                     # Try exact match first
                     key = (event_code, trigger_start_frame, 'confirm_needed')
                     found_match = self._action_range_index.get(key)

                     # If not found, try +/- 1 frame tolerance
                     if not found_match:
                         key_minus = (event_code, trigger_start_frame - 1, 'confirm_needed')
                         found_match = self._action_range_index.get(key_minus)
                     if not found_match:
                         key_plus = (event_code, trigger_start_frame + 1, 'confirm_needed')
                         found_match = self._action_range_index.get(key_plus)

                     if found_match:
                         prompt_shown = self._trigger_next_confirmation(found_match.copy())
                     # === END OPTIMIZATION ===
                    ...
                    # === OPTIMIZATION: Use index for O(1) lookup instead of O(n) linear search ===
                    key = ('NM', trigger_start_frame)
                    found_nm_range = self._action_range_index.get(key)

                    if found_nm_range:
                        prompt_shown = self._trigger_near_miss_prompt(event_to_trigger, found_nm_range.copy())
                    else:
                        print(f"Controller WARN: Could not find NM range placeholder for near miss event at F{trigger_start_frame}")
                    # === END OPTIMIZATION ===
    ...
    def _trigger_next_confirmation(self, action_range_info): # (Unchanged)
        ...
        # === AUTO-SELECT RANGE: Highlight the range being confirmed so user can easily delete it ===
        found_range_index = -1
        event_code = action_range_info.get('action')
        event_start = action_range_info.get('start')
        for i, r in enumerate(self.action_tracker.action_ranges):
            if (r.get('action') == event_code and
                r.get('start') == event_start and
                r.get('status') == 'confirm_needed'):
                found_range_index = i
                break

        if found_range_index >= 0:
            self.current_selected_index = found_range_index
            self.selected_timeline_range_data = self.action_tracker.action_ranges[found_range_index].copy()
            if self.window and self.window.timeline_widget:
                self.window.timeline_widget.set_selected_range_by_index(found_range_index)
        else:
            print(f"Controller WARN: Could not find range to auto-select for confirmation")
        # === END AUTO-SELECT ===
    ...
    def _show_confirmation_ui_internal(self, range_info): # (Unchanged)
         ...
         # Discard Confirmation button removed - Delete Range button serves same purpose
    ...
    def _trigger_near_miss_prompt(self, near_miss_event, near_miss_range): # (Unchanged)
        ...
        # === AUTO-SELECT RANGE: Highlight the NM range so user can easily delete it ===
        found_nm_index = -1
        for i, r in enumerate(self.action_tracker.action_ranges):
            if r.get('action') == 'NM' and r.get('start') == start_frame:
                found_nm_index = i
                break

        if found_nm_index >= 0:
            self.current_selected_index = found_nm_index
            self.selected_timeline_range_data = self.action_tracker.action_ranges[found_nm_index].copy()
            if self.window and self.window.timeline_widget:
                self.window.timeline_widget.set_selected_range_by_index(found_nm_index)
        else:
            print(f"Controller WARN: Could not find NM range to auto-select at F{start_frame}")
        # === END AUTO-SELECT ===
    ...
    def _show_near_miss_ui_internal(self, event_info): # (Unchanged)
        ...
        # Play audio snippet for near miss prompt
    ...
    def _play_audio_segment(self, start_time, end_time): # Audio-only snippet playback for unknown ranges
    ...
    def _cleanup_active_snippet(self): # Keep this method - still called in various places
        """Clean up temporary audio snippet files (legacy from audio-only system)"""
    ...
    # --- NEW: Range Video+Audio Playback Methods ---
    def _play_range_segment(self, start_frame, end_frame, range_index=-1):
        """Play video+audio from start_frame to end_frame, then auto-pause"""

        # Stop any existing range playback
        self._stop_range_playback()

        # Store range boundaries for monitoring
        self.playing_range_start_frame = start_frame
        self.playing_range_end_frame = end_frame
        self.playing_range_index = range_index

        # Notify timeline widget to show visual feedback
        if self.window and self.window.timeline_widget:
            self.window.timeline_widget.set_playing_range(range_index)

        # Seek to start frame
        if self.playback_manager:
            self.playback_manager.seek(start_frame)
            # Start playback
            self.playback_manager.play()
        else:
            print("Controller ERROR: PlaybackManager not available for range playback")

    def _stop_range_playback(self):
        """Stop range playback and clear boundaries"""
        if self.playing_range_end_frame is not None:

            # Clear boundaries
            self.playing_range_start_frame = None
            self.playing_range_end_frame = None
            self.playing_range_index = None

            # Clear visual feedback in timeline
            if self.window and self.window.timeline_widget:
                self.window.timeline_widget.set_playing_range(-1)

    @pyqtSlot(int, QImage)
    def _check_range_playback_boundary(self, current_frame, qimage):
        """Monitor playback and auto-pause at range end"""
        if self.playing_range_end_frame is None:
            return  # Not playing a range segment

        # Check if we've reached or passed the end frame
        if current_frame >= self.playing_range_end_frame:

            # Pause playback
            if self.playback_manager:
                self.playback_manager.pause()

            # Clear range playback state
            self._stop_range_playback()
    # --- END: Range Video+Audio Playback Methods ---

    # _handle_discard_confirmation_prompt and _handle_discard_near_miss_prompt removed - Delete Range button serves same purpose
    ...
    def _handle_timeline_delete_request(self, range_to_delete): # (Unchanged)
        ...
        try:
            ...
            # Sync timeline widget's selection with controller's selection after deletion
            if self.window and self.window.timeline_widget:
                self.window.timeline_widget.set_selected_range_by_index(self.current_selected_index)
            self._force_ui_update(self.playback_manager.current_frame)
    ...
    def _handle_timeline_range_selected(self, is_selected, range_data): # (Unchanged)
        ...
        # Clean up any playing audio snippets
        self._cleanup_active_snippet()
        ...
        ui_context = 'idle'; enable_actions = True
        if is_selected and self.selected_timeline_range_data:
            ...
            if status == 'confirm_needed':
                # Play audio-only snippet for confirmation ranges
                start_time = self.selected_timeline_range_data.get('original_event_start_time', self.selected_timeline_range_data.get('trigger_start'))
                end_time = self.selected_timeline_range_data.get('original_event_end_time', self.selected_timeline_range_data.get('trigger_end'))
                self._play_audio_segment(start_time, end_time)
            elif action_code == 'NM':
                # Play audio-only snippet for near miss ranges
                start_time = self.selected_timeline_range_data.get('original_event_start_time', self.selected_timeline_range_data.get('start_time'))
                end_time = self.selected_timeline_range_data.get('original_event_end_time', self.selected_timeline_range_data.get('end_time'))
                self._play_audio_segment(start_time, end_time)
            elif action_code == 'TBC':
                # TBC ranges don't have audio snippets - they're manually created
            else: ui_context = 'selected'; enable_actions = True # Normal range selected - no auto-playback
    ...
    @pyqtSlot()
    def _rebuild_action_range_index(self):
        """
        Rebuild the action range index for fast O(1) lookups.
        Called whenever action_ranges_changed signal fires.
        Maps (action_code, start_frame) -> range dict for instant access.
        """
        self._action_range_index = {}

        for r in self.action_tracker.action_ranges:
            action_code = r.get('action')
            start_frame = r.get('start')
            status = r.get('status')

            if action_code and start_frame is not None:
                # Create composite key for lookup
                # Use (action_code, start_frame, status) for confirm_needed ranges
                # Use (action_code, start_frame) for near-miss ranges
                if status == 'confirm_needed':
                    key = (action_code, start_frame, 'confirm_needed')
                elif action_code == 'NM':
                    key = ('NM', start_frame)
                else:
                    key = (action_code, start_frame)

                # Store reference to the range
                self._action_range_index[key] = r

    @pyqtSlot()
    def _update_timeline_widget_ranges(self):
        """Schedule a throttled timeline update (called by signals)"""
        # If timer is not active, schedule an update immediately
        if not self._timeline_update_timer.isActive():
            self._timeline_update_timer.start()
        # If timer is already running, the update will happen when it fires
        # This throttles rapid consecutive updates to max 30/sec

    def _do_timeline_update(self):
        """Actually perform the timeline widget update (throttled)"""
        print("DEBUG: _do_timeline_update executing (throttled).") # DEBUG
        if not self.window or not self.window.timeline_widget:
            print("DEBUG: _do_timeline_update - No window/timeline widget.") # DEBUG
            return

        # Check if ranges actually changed (avoid unnecessary deep copy)
        current_ranges = self.action_tracker.action_ranges
        # Use hash for faster comparison (more efficient than string conversion)
        ranges_snapshot = hash(tuple((r.get('action'), r.get('start'), r.get('end'), r.get('status'))
                                     for r in current_ranges))

        if ranges_snapshot == self._last_ranges_snapshot:
            print("DEBUG: Ranges unchanged, skipping update.") # DEBUG
            return

        self._last_ranges_snapshot = ranges_snapshot

        # Only do deep copy if ranges actually changed
        current_ranges_from_tracker = copy.deepcopy(current_ranges)
        ...
        if self.playback_manager and not self.playback_manager.is_playing:
             self._force_ui_update(self.playback_manager.current_frame)
        print("DEBUG: _do_timeline_update finished.") # DEBUG
    ...
    def _initiate_save(self): # (Unchanged)
        ...
        if self.waiting_for_confirmation or self.waiting_for_near_miss_assignment:
             if native_dialogs.ask_yes_no("Resolve Prompt?", "A confirmation or near miss assignment is pending.\nDiscard the prompt to continue saving?", default_yes=False):
                 ...
                 elif self.waiting_for_near_miss_assignment:
                     # For near miss, just clear the prompt state (user can delete NM range manually with Delete Range button)
    ...
    def _apply_restored_state(self, restored_ranges, restored_selection_index): # (Unchanged)
        ...
        # CRITICAL FIX: Timeline widget MUST be updated with restored ranges for undo/redo to work visually
        self._update_timeline_widget_ranges()
    @pyqtSlot()
