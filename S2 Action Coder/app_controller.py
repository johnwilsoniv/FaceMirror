...
class ApplicationController(QObject):
    ...
    def load_first_file(self): # (Unchanged)
        ...
        self.initial_batch_size = self.batch_processor.get_total_files()  # Capture initial count
        self.files_processed_count = 0  # Reset processed count
        print(f"Controller: Starting batch with {self.initial_batch_size} files")
    ...
    def _toggle_playback(self, should_play):
        if should_play:
            ...
            # User explicitly clicked play - clear the paused_for_prompt flag
            # This allows playback even if a prompt was previously shown
            if self.playback_manager._is_paused_for_prompt:
                print(f"Controller STUCK_GUI_DEBUG: User clicked play - clearing paused_for_prompt flag")
    ...
    def _handle_save_complete(self, success):
        ...
        # Increment processed count (whether success or failure, we attempted it)
    ...
    def _show_batch_completion_summary(self, incomplete=False):
        ...
        # Use initial_batch_size (captured at start) instead of current count
        # since files are removed as they're processed
        total_files = self.initial_batch_size
        processed_count = self.files_processed_count
