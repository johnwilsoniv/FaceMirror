...
"""
PyInstaller spec file for S2 Action Coder - Intel Mac Build
Builds a standalone application for video action coding with Whisper transcription

Usage:
    arch -x86_64 pyinstaller Action_Coder_Intel.spec

Output:
    dist/Action Coder.app  (macOS application bundle for Intel)
"""
...
# Check for FFmpeg and FFprobe binaries to bundle (x86_64 versions)
binaries = []

ffmpeg_path = Path('bin_x86_64/ffmpeg')
ffprobe_path = Path('bin_x86_64/ffprobe')

if ffmpeg_path.exists():
    binaries.append((str(ffmpeg_path), 'bin'))
    print(f"INFO: Bundling FFmpeg from {ffmpeg_path}")
else:
    print(f"WARNING: FFmpeg not found in bin_x86_64/ directory.")

if ffprobe_path.exists():
    binaries.append((str(ffprobe_path), 'bin'))
    print(f"INFO: Bundling FFprobe from {ffprobe_path}")
else:
    print(f"WARNING: FFprobe not found in bin_x86_64/ directory.")
...
if Path('patches/alignment.py').exists():
    datas += [('patches/alignment.py', 'whisperx')]
if Path('patches/vads/__init__.py').exists():
    datas += [('patches/vads/__init__.py', 'whisperx/vads')]
...
a = Analysis(
    ...
    excludes=[
        ...
        'notebook',
        'tensorflow',
        # Exclude Apple Silicon only packages
        'mlx',
        'mlx_whisper',
        'coremltools',
    ],
