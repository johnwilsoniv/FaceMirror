...
Drop-in replacement for Pure Python MTCNN using CoreML models.
Inherits all the debugged pipeline logic from pure_python_mtcnn_optimized.py.
"""
...
# Import the Pure Python MTCNN base class
sys.path.insert(0, str(Path(__file__).parent))
from pure_python_mtcnn_optimized import PurePythonMTCNN_Optimized
...
class CoreMLMTCNN(PurePythonMTCNN_Optimized):
    ...
    def __init__(self, coreml_dir=None):
        ...
        Args:
            coreml_dir: Directory containing CoreML models (default: mtcnn_models/coreml)
        """
        # Don't call super().__init__() - we'll load CoreML models instead

        if coreml_dir is None:
            coreml_dir = Path(__file__).parent / "mtcnn_models/coreml"
        else:
        ...
        # MTCNN parameters (same as Pure Python version)
...
def main():
    """Test CoreML MTCNN on sample image"""
    import cv2
    import time

    print("="*80)
    print("CoreML MTCNN Detector Test")
    print("="*80)

    # Initialize detector
    print("\n1. Initializing CoreML MTCNN...")
    detector = CoreMLMTCNN()

    # Find a test image
    patient_data_dir = Path("/Users/johnwilsoniv/Documents/SplitFace Open3/Patient Data")
    test_images = list(patient_data_dir.glob("*.jpg"))
    if not test_images:
        print(f"No test images found in {patient_data_dir}")
        return
    ...
    if len(bboxes) > 0:
        ...
        for i, bbox in enumerate(bboxes):
            print(f"     Face {i+1}: x1={bbox[0]:.1f}, y1={bbox[1]:.1f}, "
                  f"x2={bbox[2]:.1f}, y2={bbox[3]:.1f}, conf={bbox[4]:.3f}")
    ...
    output_path = Path("coreml_mtcnn_test_result.jpg")
    cv2.imwrite(str(output_path), output_img)
    print(f"   Saved to: {output_path}")

    print(f"\n{'='*80}")
    print("✅ CoreML MTCNN test complete!")
    print(f"{'='*80}")
