#!/usr/bin/env python3
"""
Baseline accuracy test: C++ OpenFace (video mode) vs Python pyCLNF (current defaults).

This establishes the current baseline after convergence parameter fixes.
"""
...
def run_python_pyclnf_current(image, bbox, verbose=True):
    """Run Python pyCLNF with current default parameters."""
    if verbose:
        print("\n" + "="*70)
        print("PYTHON pyCLNF (CURRENT DEFAULTS)")
        print("="*70)
    ...
    if verbose:
        ...
        print(f"    max_iterations: {clnf.optimizer.max_iterations}")
        print(f"    convergence_threshold: {clnf.optimizer.convergence_threshold}")
        print(f"    weight_multiplier: {clnf.weight_multiplier}")
    ...
    if verbose:
        ...
        print(f"  [3/3] Fitting landmarks (this may take 30-60s on first frame)...")
    fit_start = time.time()
    ...
    if verbose:
        ...
        print(f"\n  Results:")
        print(f"    Iterations completed: {info['iterations']}")
    ...
    for video_path in video_files:
        ...
        for frame_idx in frame_indices:
            ...
            if ret:
                ...
                frame_path = Path("test_output/baseline_test/frames") / frame_name
...
def main():
    ...
    print("="*70)
    print("C++ OpenFace (video mode) vs Python pyCLNF (current defaults)")
    ...
    for i, frame_info in enumerate(frames):
        ...
        print(f"\n  📌 Running C++ OpenFace (video mode)...")
        cpp_output_dir = Path(f"test_output/baseline_test/cpp_output/frame_{i:03d}")
        try:
        ...
        # Run Python pyCLNF (verbose only on first frame)
        print(f"  🐍 Running Python pyCLNF...")
        try:
            verbose = (i == 0)  # Verbose output only for first frame
            py_landmarks, py_info = run_python_pyclnf_current(image, bbox, verbose=verbose)
            if not verbose:
                print(f"     ✅ Python landmarks: {len(py_landmarks)} points ({py_info['iterations']} iterations)")
        except Exception as e:
    ...
    print(f"\nAccuracy Statistics (n={len(all_errors)} frames):")
    ...
    if mean_error < 5.0:
        print(f"✅ EXCELLENT: {mean_error:.3f}px mean error (target: <5px)")
    elif mean_error < 10.0:
        print(f"✅ GOOD: {mean_error:.3f}px mean error (acceptable for production)")
    else:
        print(f"⚠️  NEEDS IMPROVEMENT: {mean_error:.3f}px mean error")

    print(f"\nComparison to targets:")
    print(f"  vs PyMTCNN (16.4px):    {((16.4 - mean_error)/16.4)*100:+.1f}% better")
    print(f"  vs Previous (8.23px):   {((8.23 - mean_error)/8.23)*100:+.1f}% change")

    # Success rate
    success_count = sum(1 for e in all_errors if e < 10.0)
    success_rate = (success_count / len(all_errors)) * 100
    print(f"\nSuccess rate (<10px): {success_count}/{len(all_errors)} ({success_rate:.1f}%)")
