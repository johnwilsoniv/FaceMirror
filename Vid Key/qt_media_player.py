...
class QTMediaPlayer(QObject):
    """Enhanced media player with reliable audio/video playback and overlay support."""
    ...
    def __init__(self):
        ...
        # Create a widget to hold both video and overlay
        ...
        self.video_widget = QVideoWidget(self.container_widget)
        self.video_widget.setGeometry(0, 0, 768, 680)  # Fill the container
        self.media_player.setVideoOutput(self.video_widget)
        
        # Overlay label for text display
        self.overlay_label = QLabel(self.container_widget)
        self.overlay_label.setAlignment(Qt.AlignTop | Qt.AlignCenter)
        self.overlay_label.setStyleSheet("color: white; background-color: transparent;")
        self.overlay_label.setFont(QFont("Arial", 16, QFont.Bold))
        
        # Make the overlay label smaller to just contain the text
        self.overlay_label.setGeometry(0, 20, 768, 50)
    ...
    def set_video_path(self, video_path):
        ...
        # Adjust the overlay label size based on container width
        self.overlay_label.setGeometry(0, 20, self.container_widget.width(), 50)
    ...
    def _update_frame(self, frame_number):
        ...
        # Update the overlay text if an action is active
        if self.current_action:
            self._update_overlay_text(self.current_action)
        else:
    ...
    def _on_frame_timer(self):
        ...
        try:
            ...
            if frame != self.current_frame and frame != self.last_emitted_frame and 0 <= frame < self.total_frames:
                self.current_frame = frame
                self._update_frame(frame)
        finally:
    ...
    def set_action(self, action_code):
        """Set the current action code for overlay."""
        # Only update if the action has changed
        if self.current_action != action_code:
            self.current_action = action_code
            self._update_overlay_text(action_code)
    
    def clear_action(self):
        """Clear the current action code."""
        # Only update if there was an action set
        if self.current_action:
            self.current_action = ""
            self.overlay_label.clear()

    def _update_overlay_text(self, action_code):
        """Update the overlay label with action text."""
        if not action_code:
            self.overlay_label.clear()
            return
        ...
        if action_text:
            ...
            display_text = f"{action_code}"

        # Set text with text shadow but no background
        self.overlay_label.setText(f"""<span style="
            color: white;
            font-size: 16pt;
            font-weight: bold;
        ...
        # Ensure the overlay label itself is transparent
