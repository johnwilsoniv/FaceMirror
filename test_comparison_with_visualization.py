#!/usr/bin/env python3
"""
Comprehensive test of Python PyFaceAU vs C++ OpenFace 2.2.
Generates side-by-side comparison images: Python (left) vs C++ OpenFace (right)
"""
...
def parse_openface_landmarks(row):
    ...
    OpenFace CSV format: x_0, x_1, ..., x_67, y_0, y_1, ..., y_67
    """
    landmarks = []
...
def generate_side_by_side_comparison(python_results, cpp_results, video_path, output_path):
    """
    Generate side-by-side comparison images: Python (left) vs C++ OpenFace (right).
    3 frames per video showing interesting cases.
    """
    py_results = python_results['results']
    cpp_df = cpp_results['df']

    # Reload video to get frames
    cap = cv2.VideoCapture(str(video_path))
    if not cap.isOpened():
        raise ValueError(f"Failed to open video: {video_path}")
    ...
    if fallback_indices:
        # Take first, middle, last fallback frame
    ...
    if len(frame_indices_to_show) < 3:
        ...
        if val_fail_indices:
            ...
            # Take evenly distributed validation failures
...
def main():
    ...
    print("="*80)
    print("PYTHON vs C++ OPENFACE SIDE-BY-SIDE COMPARISON")
    print("="*80)
    ...
    if len(problem_files) > 0:
        print(f"\n🔴 {len(problem_files)} files requiring MTCNN fallback:")
        for _, row in problem_files.iterrows():
            print(f"  • {row['video']}: {row['py_mtcnn_usage']:.1f}% MTCNN usage")
    else:
        print("\n✅ No MTCNN fallback needed")

    validation_failures = df_summary[df_summary['py_validation_failures'] > 0]
    if len(validation_failures) > 0:
        print(f"\n⚠️  {len(validation_failures)} files with validation failures:")
        for _, row in validation_failures.iterrows():
            print(f"  • {row['video']}: {row['py_validation_failures']} frames failed")
